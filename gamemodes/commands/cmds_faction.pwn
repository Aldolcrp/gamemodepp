...
CMD:fhelp(playerid, params[])
{
	...
	switch(AccountData[playerid][pFaction])
	{
		case 1: // Polisi
		{
			ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""TTR"Astral City Roleplay "WHITE"- Polisi Command", 
			"Perintah\tKeterangan\n\
		...
		case 2: // Pemerintah
		{
			ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""TTR"Astral City Roleplay "WHITE"- Pemerintah Command",
			"Perintah\tKeterangan\n\
		...
		case 3: //ems
		{
			ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""TTR"Astral City Roleplay "WHITE"- EMS Command",
			"Perintah\tKeterangan\n\
		...
		case 5: // Bengkel
		{
			ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""TTR"Astral City Roleplay "WHITE"- Bengkel Command",
			"Perintah\tKeterangan\n\
...
Dialog:DialogTrace(playerid, response, listitem, inputtext[])
{
	...
	switch(listitem)
	{
		case 0:// nomor hp
		{
			Dialog_Show(playerid, TrackNoPhone, DIALOG_STYLE_INPUT, ""TTR"Astral City Roleplay "WHITE"- Track Telepon", "Mohon masukkan nomor telepon valid\nContoh: 55261, 02521\n"YELLOW"(Mohon masukkan dikolom bawah sini):", "Track", "Batal");
		}
		case 1:// Plate Nomor
		{
			Dialog_Show(playerid, TrackPlateNomor, DIALOG_STYLE_INPUT, ""TTR"Astral City Roleplay "WHITE"- Track Plate", "Mohon masukkan nomor plate kendaraan valid\nContoh: VR 5233 BOS, VR 2122 DAN\n"YELLOW"(Mohon masukkan dikolom bawah sini):", "Track", "Batal");
		}
		case 2:// Akun Twitter
		{
			Dialog_Show(playerid, TrackAkunTwitter, DIALOG_STYLE_INPUT, ""TTR"Astral City Roleplay "WHITE"- Track Sosial Media", "Mohon memasukkan username akun media Twitter yang ingin dilacak\n"YELLOW"(Masukkan dibawah sini dengan Username yang Valid):", "Track", "Batal");
		}
	}
	return 1;
}
Dialog:TrackAkunTwitter(playerid, response, listitem, inputtext[])
{
	...
	if(isnull(inputtext))
		return Dialog_Show(playerid, TrackAkunTwitter, DIALOG_STYLE_INPUT, ""TTR"Astral City Roleplay "WHITE"- Track Sosial Media", "ERROR: Tidak dapat Kosong!\nMohon memasukkan username akun media Twitter yang ingin dilacak\n"YELLOW"(Masukkan dibawah sini dengan Username yang Valid):", "Track", "Batal");
...
Dialog:TrackNoPhone(playerid, response, listitem, inputtext[])
{
	if(!response) return 1;
	if(isnull(inputtext))
		return Dialog_Show(playerid, TrackNoPhone, DIALOG_STYLE_INPUT, ""TTR"Astral City Roleplay "WHITE"- Track Telepon", "Mohon masukkan nomor telepon valid\nContoh: 55261, 02521\n"YELLOW"(Mohon masukkan dikolom bawah sini):", "Track", "Batal");
...
Dialog:TrackPlateNomor(playerid, response, listitem, inputtext[])
{
	...
	if(isnull(inputtext) || IsNumeric(inputtext))
		return Dialog_Show(playerid, TrackPlateNomor, DIALOG_STYLE_INPUT, ""TTR"Astral City Roleplay "WHITE"- Track Plate", "Mohon masukkan nomor plate kendaraan valid\nContoh: VR 5233 BOS, VR 2122 DAN\n"YELLOW"(Mohon masukkan dikolom bawah sini):", "Track", "Batal");
...
CMD:makesks(playerid, params[])
{
	if(AccountData[playerid][pFaction] != FACTION_EMS)
		return ShowTDN(playerid, NOTIFICATION_ERROR, "Anda bukan bagian dari Mitra EMS Astral City!");
...
{
	new minsty[512];
	if(AccountData[playerid][pFaction] != FACTION_EMS) return ShowTDN(playerid, NOTIFICATION_ERROR, "Anda bukan bagian dari EMS Kota Astral City!");
	new string[128];
...
CMD:g(playerid, params[])
{
	...
	if(AccountData[playerid][pFaction] == FACTION_NONE)
		return ShowTDN(playerid, NOTIFICATION_ERROR, "Anda tidak termasuk dalam factions!");

	if(isnull(params))
		return ShowTDN(playerid, NOTIFICATION_SYNTAX, "/ro [pesan]");

	new lstr[256], factionname[32], color[16];

	switch (AccountData[playerid][pFaction])
	{
		case FACTION_POLISI:
		{
			format(factionname, sizeof(factionname), "POLISI");
			format(color, sizeof(color), "{201ee5}");
		}
		case FACTION_PEMERINTAH:
		{
			format(factionname, sizeof(factionname), "PEMERINTAH");
			format(color, sizeof(color), "{1cecca}");
		}
		case FACTION_EMS:
		{
			format(factionname, sizeof(factionname), "EMS");
			format(color, sizeof(color), "{ff0000}");
		}
		case FACTION_TRANS:
		{
			format(factionname, sizeof(factionname), "TRANS");
			format(color, sizeof(color), "{ffff00}");
		}
		case FACTION_BENGKEL:
		{
			format(factionname, sizeof(factionname), "BENGKEL");
			format(color, sizeof(color), "{f20f7e}");
		}
		case FACTION_PEDAGANG:
		{
			format(factionname, sizeof(factionname), "PEDAGANG");
			format(color, sizeof(color), "{f38b03}");
		}
		default:
		{
			format(factionname, sizeof(factionname), "FACTION");
			format(color, sizeof(color), "{ffffff}");
		}
	}

	format(lstr, sizeof(lstr), "%s[Radio Lintas Faction] %s | %s: "WHITE"%s",
		color, factionname, ReturnName(playerid), params);

	foreach (new i : Player)
	{
		if(AccountData[i][IsLoggedIn] && AccountData[i][pFaction] != FACTION_NONE)
		{
			SendClientMessageEx(i, -1, lstr);
		}
	}
	return 1;
}
