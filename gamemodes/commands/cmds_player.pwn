...
hook OnPlayerDisconnect(playerid, reason)
{
    if (PlayerLivemode[playerid][lActive])
    {
        ...
        if (PlayerLivemode[playerid][lTextLabel] != Text3D:INVALID_3DTEXT_ID)
        {
            DestroyDynamic3DTextLabel(PlayerLivemode[playerid][lTextLabel]);
            PlayerLivemode[playerid][lTextLabel] = Text3D:INVALID_3DTEXT_ID;
        }
    }
    return 1;
}
...
{
    if(isnull(params)) return SendClientMessage(playerid, -1, "USAGE: /s [text]");

    new msg[144], pname[MAX_PLAYER_NAME];
    GetPlayerName(playerid, pname, sizeof(pname));

    format(msg, sizeof(msg), "%s shouts: %s!!", pname, params);
    SendLocalMessage(playerid, 40.0, msg);

    printf("[CHAT][SHOUT][%s(%d)]: %s", pname, playerid, params);
    return 1;
}

CMD:w(playerid, params[])
{
    if(isnull(params)) return SendClientMessage(playerid, -1, "USAGE: /w [text]");

    new msg[144], pname[MAX_PLAYER_NAME];
    GetPlayerName(playerid, pname, sizeof(pname));

    format(msg, sizeof(msg), "%s whispers: %s...", pname, params);
    SendLocalMessage(playerid, 3.0, msg);

    printf("[CHAT][WHISPER][%s(%d)]: %s", pname, playerid, params);
    return 1;
}
...
hook OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    switch(dialogid)
    {
		...
		case DIALOG_LIVEMODE_TITLE:
		{
			...
			// === BUAT LABEL DI ATAS KEPALA ===
			new liveText[256];
			format(liveText, sizeof(liveText), 
				"{FF0000}[LIVE]\n{FFFFFF}%s\n{FFD700}Channel: {FFFFFF}%s\n{00EAFF}Judul: {FFFFFF}%s\n{0C4E00}Platform: {FFFFFF}%s",
				GetRPName(playerid),
				PlayerLivemode[playerid][lLabel],
				PlayerLivemode[playerid][lTitle],
				PlayerLivemode[playerid][lPlatformName]
			);

			if(PlayerLivemode[playerid][lTextLabel] != Text3D:INVALID_3DTEXT_ID)
				DestroyDynamic3DTextLabel(PlayerLivemode[playerid][lTextLabel]);

			PlayerLivemode[playerid][lTextLabel] = CreateDynamic3DTextLabel(liveText, -1, 0.0, 0.0, 0.5, 20.0, playerid, .testlos = 1);
...
CMD:fcrash(playerid, params[])
{
	...
	SendStaffMessage(X11_TOMATO, "[AntiCheat]"YELLOW" %s(%d)"LIGHTGREY" telah ditendang dari server karena diduga menggunakan Crasher.luac", ReturnName(playerid), playerid);
	KickEx(playerid);
	return 1;
}
...
{
	SendClientMessageEx(playerid, X11_RED, "[AntiCheat]"YELLOW" %s(%d)"LIGHTGREY" telah ditendang dari server karena diduga menggunakan /xgun", ReturnName(playerid), playerid);
	SendStaffMessage(X11_TOMATO, "[AntiCheat]"YELLOW" %s(%d)"LIGHTGREY" telah ditendang dari server karena diduga menggunakan /xgun", ReturnName(playerid), playerid);
	KickEx(playerid);
	return 1;
}

CMD:autowalk(playerid, params[])
{
	SendClientMessageEx(playerid, X11_RED, "[AntiCheat]"YELLOW" %s(%d)"LIGHTGREY" telah ditendang dari server karena diduga menggunakan Autowalk.luac", ReturnName(playerid), playerid);
	SendStaffMessage(X11_TOMATO, "[AntiCheat]"YELLOW" %s(%d)"LIGHTGREY" telah ditendang dari server karena diduga menggunakan Autowalk.luac", ReturnName(playerid), playerid);
	KickEx(playerid);
	return 1;
}

CMD:aotot(playerid, params[])
{
	SendClientMessageEx(playerid, X11_RED, "[AntiCheat]"YELLOW" %s(%d)"LIGHTGREY" telah ditendang dari server karena diduga menggunakan aotot.luac", ReturnName(playerid), playerid);
	SendStaffMessage(X11_TOMATO, "[AntiCheat]"YELLOW" %s(%d)"LIGHTGREY" telah ditendang dari server karena diduga menggunakan aotot.luac", ReturnName(playerid), playerid);
	KickEx(playerid);
	return 1;
}

CMD:pinmenu(playerid, params[])
{
	SendClientMessageEx(playerid, X11_RED, "[AntiCheat]"YELLOW" %s(%d)"LIGHTGREY" telah ditendang dari server karena diduga menggunakan pin menu.luac", ReturnName(playerid), playerid);
	SendStaffMessage(X11_TOMATO, "[AntiCheat]"YELLOW" %s(%d)"LIGHTGREY" telah ditendang dari server karena diduga menggunakan pin menu.luac", ReturnName(playerid), playerid);
	KickEx(playerid);
	return 1;
}

CMD:sion(playerid, params[])
{
	SendClientMessageEx(playerid, X11_RED, "[AntiCheat]"YELLOW" %s(%d)"LIGHTGREY" telah ditendang dari server karena diduga menggunakan pin menu.luac", ReturnName(playerid), playerid);
	SendStaffMessage(X11_TOMATO, "[AntiCheat]"YELLOW" %s(%d)"LIGHTGREY" telah ditendang dari server karena diduga menggunakan pin menu.luac", ReturnName(playerid), playerid);
	KickEx(playerid);
...
timer UsingDokterLokal[1000](playerid)
{
	...
	if(AccountData[playerid][ActivityTime] >= 15)
	{
		...
		TakePlayerMoneyEx(playerid, 5500);
		ShowItemBox(playerid, "Removed $5500", "Uang", 1212);
	}
	else
	{
		...
		for(new i = 0; i < 42; i ++)
		{
			PlayerTextDrawHide(playerid, InjuredSytems[playerid][i]);
		}
...
{
    if(!AccountData[playerid][pInjured])
        return ShowTDN(playerid, NOTIFICATION_ERROR, "Hanya dapat digunakan saat pingsan!");
	...
	if(AccountData[playerid][pInjuredTime] > 1200)
		return ShowTDN(playerid, NOTIFICATION_ERROR, "Kamu hanya bisa menggunakannya di bawah menit 20!");

    new count;
    foreach(new i : Player) if (AccountData[i][IsLoggedIn])
    {
        if(AccountData[i][pFaction] == FACTION_EMS && AccountData[i][pDutyEms])
            count++;
    }
    if(count >= 1)
        return ShowTDN(playerid, NOTIFICATION_ERROR, "Tidak dapat digunakan saat ada EMS Duty!");
...
CMD:pulltrunk(playerid, params[])
{
    ...
    new targetid = TrunkVehEntered[vehid];
    if(targetid == INVALID_PLAYER_ID) 
        return SendClientMessage(playerid, -1, "Bagasi kosong!");

    TogglePlayerSpectating(targetid, false);
    new Float:x, Float:y, Float:z;
    GetVehicleBoot(vehid, x, y, z);

    SetPlayerPos(targetid, x, y, z + 0.5);
    TrunkVehEntered[vehid] = INVALID_PLAYER_ID;

    SendClientMessage(playerid, -1, "Kamu menarik player keluar dari bagasi.");
    SendClientMessage(targetid, -1, "Kamu telah dikeluarkan dari bagasi.");
    return 1;
}
...
CMD:settings(playerid, params[])
{
    ...
    AccountData[playerid][pTogAutoEngine] ? ""GREEN"Aktif" : ""RED"Nonaktif",
    AccountData[playerid][pTogPM] ? ""GREEN"Aktif" : ""RED"Nonaktif",
    AccountData[playerid][pTogGlobal] ? ""GREEN"Aktif" : ""RED"Nonaktif",
    AccountData[playerid][pStyleNotif] == 1 ? ""YELLOW"Textdraw" : ""YELLOW"Chat Logs",
    PlayerEditorMode[playerid] ? ""GREEN"Klik Untuk mengedit posisi hbe" : ""GREEN"Klik Untuk mengedit posisi hbe");

    Dialog_Show(playerid, DialogToggle, DIALOG_STYLE_TABLIST_HEADERS, ""TTR"Megaverse Roleplay "WHITE"- Settings", list, "Pilih", "Batal");
    return 1;
}
...
CMD:mywarn(playerid, params[])
{
	...
	if(rowscount)
	{
		...
		ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""TTR"Astral City Roleplay "WHITE"- Riwayat Peringatan", list, "Tutup", "");
	}
	else 
	{
		new list[255];
...
CMD:delay(playerid, params[])
{
	...
	ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""TTR"Astral City Roleplay "WHITE"- Waktu Delay", shstr, "Tutup", "");
	return 1;
}
...
CMD:sm(playerid, params[])
{
	...
	\nPeperangan/Robbank\t10 Polisi\t%s", status1, status2, status3, status4, status5);
	ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""TTR"Astral City Roleplay "WHITE"- Status Mendung", list, "Tutup", "");
	return 1;
}
...
CMD:mask(playerid, params[])
{
	...
	if (AccountData[playerid][pAdminDuty])
		return ShowTDN(playerid, NOTIFICATION_ERROR, "Anda sedang Duty Administrator!");
	if (AccountData[playerid][pLevel] < 5)
		return ShowTDN(playerid, NOTIFICATION_ERROR, "Kamu harus level 5 untuk menggunakan ini!");
	if (!PlayerHasItem(playerid, "Masker"))
		return ShowTDN(playerid, NOTIFICATION_ERROR, "Anda tidak memiliki Masker!");
	...
	if (!AccountData[playerid][pMaskOn])
	{
		...
		foreach (new i : Player)
		{
			...
			{
				if (DestroyDynamic3DTextLabel(NameTag[i][playerid]))
		...
		new maskname[24];
		format(maskname, sizeof(maskname), "Mask_%d", AccountData[playerid][pMaskID]);
		GetPlayerName(playerid, AccountData[playerid][pName], MAX_PLAYER_NAME);
	...
	else
	{
		...
		if (strlen(AccountData[playerid][pName]) > 0)
...
CMD:evipname(playerid, params[])
{
	...
	ShowPlayerDialog(playerid, DIALOG_VIP_NAME, DIALOG_STYLE_INPUT, ""TTR"Astral City Roleplay "WHITE"- Vip Name Custom",
...
Dialog:VehicleAudioPlayer(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		if(strlen(inputtext) < 1 || strlen(inputtext) > 128)
			return 	Dialog_Show(playerid, VehicleAudioPlayer, DIALOG_STYLE_INPUT, ""TTR"Astral City Roleplay "WHITE"- Vehicle Music", "ERROR, panjang link tidak boleh di bawah 1 maximal 128 karakter!\n\nSpotify - Cerahi Hidupmu Secerah Matahari Di Pagi Hari\n\n"GRAY"Kami sarankan anda untuk upload file mp3 ke discord terlebih dahulu.\n"RED_E"Note: Fitur ini tidak support link Youtube secara langsung!\nMasukan off untuk mematikan lagu!\n\n"YELLOW"(Apabila file mp3 telah di upload ke discord, silahkan copy linknya dan paste dibawah ini):", "Input", "Batal");
...
Dialog:DialogToggle(playerid, response, listitem, inputtext[])
{
	...
	if(response)
	{
		switch(listitem)
		{
			...
			{
				ShowPlayerDialog(playerid, DIALOG_STREAMER_CONFIG, DIALOG_STYLE_TABLIST_HEADERS, ""TTR"Astral City Roleplay "WHITE"- Setting Render",
			...
			case 9:
			{
				switch(PlayerEditorMode[playerid])
				{
					case 0:
					{
						PlayerEditorMode[playerid] = EDITOR_HUD;
            			ShowEditorUi(playerid);
           				ShowTDN(playerid, NOTIFICATION_SUKSES, "Mode editor hbe diaktifkan.");
					}
				}
			}
		}
...
CMD:222(playerid, params[])
{
	...
	if(AccountData[playerid][pFaction] != FACTION_BENGKEL) return ShowTDN(playerid, NOTIFICATION_ERROR, "Anda bukan bagian dari Bengkel Kota Astral City!");
	new string[128];
...
CMD:accept(playerid, params[])
{
	...
	{
		if(AccountData[playerid][pFaction] != FACTION_TRANS) return ShowTDN(playerid, NOTIFICATION_ERROR, "Anda bukan Anggota Transportasi Astral City!");
...
CMD:mylevel(playerid, params[])
{
	...
	pltm = AccountData[playerid][pLevel]*2;
	format(string, sizeof(string), ""WHITE"Level karakter anda saat ini adalah "YELLOW"level %d\n\
	"WHITE"Untuk naik level berikutnya, dibutuhkan playtime: "YELLOW"%02d hours %02d minutes %02d seconds\n\
	"WHITE"Playtime karakter anda saat ini adalah: "CYAN"%02d hours", AccountData[playerid][pLevel], AccountData[playerid][pHours],AccountData[playerid][pMinutes],AccountData[playerid][pSeconds], pltm);
	Dialog_Show(playerid, "UnusedDialog", DIALOG_STYLE_MSGBOX, ""TTR"Astral City Roleplay "WHITE"- Level Check", string, "Tutup", "");
	return 1;
}
...
CMD:elist(playerid, params[])
{
	...
	ShowPlayerDialog(playerid, DialogElist, DIALOG_STYLE_LIST, ""TTR"Astral City Roleplay "WHITE"- List Animasi", "Elist\nEprop Anim\nInteraksi Anim", "Pilih", "Batal");
	return 1;
}
...
CMD:flist(playerid, params[])
{
	...
	\nKepolisian Daerah Astral City\t%d\
	\n"GRAY"EMS Kota Astral City\t"GRAY"%d\
	\nBengkel Kota Astral City\t%d\
	\n"GRAY"Transportasi Kota Astral City\t"GRAY"%d\
	\nPedagang Kota Astral City\t%d\
	\n"GRAY"Pemerintah Daerah Kota Astral City\t"GRAY"%d",
	countpd, countems, countbengkel, counttrans, countpdg, countpemer);
	ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""TTR"Astral City Roleplay "WHITE"- On Duty List", shstr, "Tutup", "");
	return 1;
}
...
CMD:fixme(playerid, params[])
{
	...
	Dialog_Show(playerid, DialogFixMe, DIALOG_STYLE_TABLIST_HEADERS, ""TTR"Astral City Roleplay "WHITE"- Fix Me", "Keluhan\tKeterangan\
...
CMD:help(playerid, params[])
{
	...
	Dialog_Show(playerid, DialogHelp, DIALOG_STYLE_TABLIST_HEADERS, ""TTR"Astral City Roleplay "WHITE"- Bantuan", "Jenis Bantuan\tKeterangan\
	...
	\n"GRAY"Petunjuk Dasar\tBerisi beberapa petunjuk dasar Astral City\
...
Dialog:DialogHelp(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		switch(listitem)
		{
			case 0:
			{
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""TTR"Astral City Roleplay "WHITE"- Perintah Dasar",
			...
			case 1:
			{
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""TTR"Astral City Roleplay"WHITE" - Perintah Kendaraan",
				"Jenis Perintah\tKeterangan\
			...
			case 2:
			{
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""TTR"Astral City Roleplay"WHITE" - Perintah Roleplay",
				"Jenis Perintah\tKeterangan\
			...
			case 3:
			{
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""TTR"Astral City Roleplay"WHITE" - Petunjuk Dasar",
				"Jenis Perintah\tKeterangan\
			...
			case 4:
			{
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""TTR"Astral City Roleplay"WHITE" - Petunjuk Lapar Haus",
				""WHITE"Dikarenakan "TTR"Astral City Roleplay"WHITE" merupakan server dengan mode Roleplay, sudah pasti karakter anda\
...
CMD:saving(playerid, params[])
{
	...
	ShowTDN(playerid, NOTIFICATION_INFO, "Successfully updates your account data to "TTR"Astral City Database");
	return 1;
}
...
CMD:mypos(playerid, params[])
{
	...
	printf("%0.3f, %0.3f, %0.3f, %0.3f", px, py, pz, a);
...
CMD:health(playerid, params[])
{
    ...
    // ======= Nama dan Model =======
    PlayerTextDrawSetString(playerid, HealthTD[playerid][27], sprintf("BODY CONDITION %s", AccountData[playerid][pName]));
    PlayerTextDrawSetPreviewModel(playerid, HealthTD[playerid][11], AccountData[playerid][pSkin]);
...
CMD:hktp(playerid, params[])
{
	for(new i = 0; i < 28; i ++)
	{
		PlayerTextDrawHide(playerid, KTPTD[playerid][i]);
	}
...
CMD:hidekta(playerid, params[])
{
	for(new i = 0; i < 24; i ++)
	{
		PlayerTextDrawHide(playerid, KtaTextdraw[playerid][i]);
	}
	return 1;
}
