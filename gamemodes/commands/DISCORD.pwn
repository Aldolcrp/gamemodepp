...
public DCC_DM_EMBED(str[], pin, id[])
{
	...
	PM = DCC_GetCreatedPrivateChannel();
	new DCC_Embed:embed = DCC_CreateEmbed(.title="Astral City Roleplay", .image_url="https://media.discordapp.net/attachments/1159376834337701941/1160296311287988325/A4.png?ex=65342525&is=6521b025&hm=287210874fb4c4797d73e750d1c9da1d9a8c39f9873617d5f1a38e56127a2c9b&=&width=607&height=607", .footer_text = "Pemberitahuan Tiket Astral City Roleplay");
...
public DCC_DM_LUPAPW(pin, id[])
{
	...
	new DCC_Embed:embed = DCC_CreateEmbed(.title="Pemulihan Akun - Astral City Roleplay", .image_url="https://media.discordapp.net/attachments/1159376834337701941/1160296311287988325/A4.png?ex=65342525&is=6521b025&hm=287210874fb4c4797d73e750d1c9da1d9a8c39f9873617d5f1a38e56127a2c9b&=&width=669&height=669", .footer_text = "Astral City Roleplay #1");
	new str1[1000];
...
public CheckAccountUCP(DiscordID[])
{
	...
	new DCC_Embed:regist = DCC_CreateEmbed(.footer_text = "Penjaga Kota Astral City Roleplay", .thumbnail_url = "https://media.discordapp.net/attachments/1159376834337701941/1160296311287988325/A4.png?ex=65342525&is=6521b025&hm=287210874fb4c4797d73e750d1c9da1d9a8c39f9873617d5f1a38e56127a2c9b&=&width=669&height=669");
...
public CheckDiscordUCP(DiscordID[], Nama_UCP[])
{
	...
	if(cache_num_rows())
	{
		SendEmbedMessage("Astral City Roleplay", "> :x1: ** Nama UCP ** tersebut sudah ada dalam Database!\n> Gunakan ** Nama UCP ** yang lain", "#MainROLEPLAY", "", "", "1159376835772153862", 0xf0635e);
		return 1;
	}
	else 
	{
...
public CheckDiscordPlayer(DiscordID[])
{
	if(!cache_num_rows())
	{
		SendEmbedMessage("Astral City Roleplay", "> :x1: Anda belum pernah membuat UCP Sebelumnya!", "Penjaga Kota Astral City Roleplay", "", "", "1160993063234179102", 0xff0000);
	}
	else
	{
		new query[200];
...
public CheckDiscordID(DiscordID[], Nama_UCP[])
{
	...
	if(cache_num_rows())
	{
		...
		SendEmbedMessage("Astral City Roleplay!", frmxt, "#MainROLEPLAY", "", "", "1159376835772153862", 0xf0635e);
		return 1;
	}
	else 
	{
...
DCMD:icmsg(user, channel, params[])
{
	...
	{
		SendEmbedMessage("SYNTAX USAGE", "!icmsg [pesan]", "Astral City Roleplay #1", "", "", "1159376834337701941", 0x707371);
		return 1;
	}
	...
	SendEmbedMessage("MESSAGE TO SERVER", shstr, "Astral City Roleplay #1", "", "", "1159376834337701941", 0x707371);
	return 1;
}
...
DCMD:update(user, channel, params[])
{
    ...
    {
        SendEmbedMessage("ERROR", "Hanya Founder yang dapat menggunakan discord command ini!", "Astral City Roleplay #Updates", "", "", "1010976548326756422", 0x3bd17c);
        return 1;
    }
	
	if(isnull(params))
	{
		return SendEmbedMessage("SYNTAX USAGE", "!update [list update]", "Astral City Roleplay #Updates", "", "", "1010976548326756422", 0x3bd17c);
	}

	format(shstr, sizeof(shstr), "%s", params);
	SendEmbedMessage("UPDATES", shstr, "Astral City Roleplay #Updates", "https://cdn.discordapp.com/attachments/1063836454586962020/1111675562428207165/ASTRALCITY_1.jpg", "", "1010976548326756422", 0x3bd17c);
	return 1;
}
...
DCMD:players(user, channel, params[])
{
	...
	new DCC_Embed:leave = DCC_CreateEmbed(.title = "Astral City Roleplay", .footer_text = "Penjaga Pintu Kota #1");
	format(dc, sizeof(dc), "**Pemain yang sedang dikota Astral City Roleplay saat ini:**\n**Jumlah:** %d", Iter_Count(Player));
