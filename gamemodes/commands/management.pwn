...
FUNC:: ScheduleMTExecute()
{
    if(g_Schedule)
	{
		if(g_ScheduleTime != 0 && g_ScheduleTime <= gettime())
		{
			...
			SaveAll();
			SendClientMessageToAllEx(X11_RED, "AdmCmd: Server Telah membackup semua data pemain dan kendaraan ke "PINK1"Astral City Database");
...
public DeleteCharQuery(playerid, UCP[])
{
	...
	else
	{
		...
		Dialog_Show(playerid, DeleteCharList, DIALOG_STYLE_LIST, sprintf(""TTR"Astral City Roleplay "WHITE"- Char %s", UCP), sha, "Delete", "Cancel");
	}
	return 1;
}
...
Dialog:DeleteCharList(playerid, response, listitem, inputtext[])
{
	...
	if(cache_num_rows())
	{
		...
		if(listitem >= 0 && listitem < cache_num_rows())
		{
			...
			ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""TTR"Astral City Roleplay "WHITE"- Delete Char", kckstr, "Tutup", "");
