//===============================GAMEMMODE Acrp DEVELOPMENT RAYYZIX===============================//
...
enum
{
	...
	DANN_DYNHELP,
	DIALOG_BRANKAS_MENU,
	DIALOG_BRANKAS_DEPOSIT,
	DIALOG_BRANKAS_WITHDRAW,
	DIALOG_BRANKAS_WITHDRAW_AMOUNT,
	DIALOG_BRANKAS_DEPOSIT_AMOUNT,
    ...
    DIALOG_INVOICE_COST,
    DIALOG_PAY_INVOICE,
	DIALOG_GIVE_MONEY_PLAYER,
	DIALOG_GIVE_MONEY_AMOUNT,
	DIALOG_MENU_PAYMENT,
...
stock const Float: SpawnBandara[][] = {
	{1694.499, -2241.657, 13.539, 175.105},
	{1678.214, -2241.551, 13.549, 181.685}
};
...
#include "Modules/Bombomcar/bombomcar.pwn"
#include "Modules/systems/Pickup.pwn"
#include "Modules/robbank/Robbank.pwn"
#include "Modules/systems/JobVehicles.pwn"
#include "Modules/systems/systems_tax.pwn"
#include "Modules/systems/systems_lockpick.pwn"
...
#include "Modules/toggle_ui/Toggle_ui.pwn"
#include "Modules/toggle_ui/Function.pwn"
#include "Modules/Race/race_funcs.pwn"
#include "Modules/Race/ui_race.pwn"
#include "Modules/Universitas/universitas_surat.pwn"
...
#include "Modules/PlayerStuff/player_scane.pwn"
...
#include "Modules/systems/minigame_repairkit.pwn"
...
// ------------------------------------------
//#include "Modules/user-interface/notifikasi/Header"
//#include "Modules/user-interface/notifikasi/Function"
...
#include "Modules/user-interface/ui_globalinsurance.pwn"
#include "Modules/user-interface/ui_compas.pwn"
#include "Modules/user-interface/ui_newbie.pwn"
#include "Modules/user-interface/ui_menupedagang.pwn"
#include "Modules/user-interface/ui_radial.pwn"

#include "Modules/Dynamic/Dynamic_Rusun/rusun_functions.pwn"
#include "Modules/Dynamic/Dynamic_House/dyn_house.pwn"
#include "Modules/Dynamic/Dynamic_Brankas/dyn_brankas.pwn"
...
#include "Modules/systems/systems_spawn.pwn"
...
//#include "Modules/showroom/showroom_grid_ui.pwn"
...
stock DatabaseConnection()
{
    ...
    {
        print("Astral City Roleplay: Connection to MySQL FAILED! Shutting down...");
        SendRconCommand("exit");
        return 0;
    }

    print("Astral City Roleplay: Successfully connected to MySQL.");
    return 1;
}
...
public OnPlayerClickPlayer(playerid, clickedplayerid, source)
{
	if(SQL_IsCharacterLogged(playerid) && AccountData[playerid][pAdmin] > 0)
	{
		...
		new title[127];
		format(title, sizeof(title), ""TTR"Astral City Roleplay "WHITE"- %s(%d)", ReturnName(clickedplayerid), clickedplayerid);
...
public OnPlayerRequestClass(playerid, classid)
{
	...
	SetPlayerCameraPos(playerid, 461.66, -1976.75, 84.47);
	SetPlayerCameraLookAt(playerid, 461.43, -1975.85, 84.10);
	InterpolateCameraPos(playerid,461.66, -1976.75, 84.47, 1048.71, -1935.98, 105.12,30000, CAMERA_MOVE);
    InterpolateCameraLookAt(playerid,461.43, -1975.85, 84.10,  1048.37, -1935.11, 104.76,30000, CAMERA_MOVE);
...
public OnPlayerText(playerid, text[])
{
    ...
    new msg[144], pname[MAX_PLAYER_NAME];
    GetPlayerName(playerid, pname, sizeof(pname));

    printf("[CHAT][IC][%s(%d)]: %s", pname, playerid, text);

    if(AccountData[playerid][pAdmin] > 0 && AccountData[playerid][pAdminDuty])
    {
        if(strlen(text) > 64)
        {
            SendNearbyMessage(playerid, 15.0, -1,
                "Admin "RED"%s"WHITE": (( %.64s...", AccountData[playerid][pAdminname], text);
            SendNearbyMessage(playerid, 15.0, -1,
                "...%s ))", text[64]);
        }
        else 
        {
            SendNearbyMessage(playerid, 15.0, -1,
                "Admin "RED"%s"WHITE": (( %s ))", AccountData[playerid][pAdminname], text);
        }
        return 0; 
    }

    format(msg, sizeof(msg), "%s says: %s", pname, text);
    SendLocalMessage(playerid, 15.0, msg);
...
public OnPlayerConnect(playerid)
{
	...
	PlayAudioStreamForPlayer(playerid, "http://k.top4top.io/m_3038s2tdh1.mp3", 0.0, 0.0, 0.0, 50.0, 0);

	AccountData[playerid][PlaySessionStart] = gettime();

	if (g_AsuransiAll && g_AsuransiTime > 0)
	{
		for (new i = 0; i < sizeof(Insu::textdraw); i++)
		{
			TextDrawShowForPlayer(playerid, Insu::textdraw[i]);
		}
	...
	if(AccountData[playerid][pSpawned]) {
		...
		else if(key == 90)
		{
			...
			switch(voiceMode)
			{
				case 1:
				{
					PlayerTextDrawHide(playerid, HbeNewv2[playerid][28]);
					PlayerTextDrawShow(playerid, HbeNewv2[playerid][29]);
					PlayerTextDrawShow(playerid, HbeNewv2[playerid][30]);
					CallRemoteFunction("UpdatePlayerVoiceDistance", "ddf", playerid, voiceMode, 15.0);
				}
				case 2:
				{
					PlayerTextDrawShow(playerid, HbeNewv2[playerid][28]);
					PlayerTextDrawShow(playerid, HbeNewv2[playerid][30]);
					PlayerTextDrawShow(playerid, HbeNewv2[playerid][29]);
				...
				{
					PlayerTextDrawShow(playerid, HbeNewv2[playerid][30]);
					PlayerTextDrawHide(playerid, HbeNewv2[playerid][29]);
					PlayerTextDrawHide(playerid, HbeNewv2[playerid][28]);
...
public OnPlayerSpawn(playerid)
{
	...
	for(new i = 0; i < 13; i ++) PlayerTextDrawShow(playerid, InfoDate[playerid][i]);
	...
	else
	{
		if(!AccountData[playerid][pSpawned])
		{
			...
			SendClientMessage(playerid, -1, ""TTR"[Server]: "WHITE"Discord kita yaitu: "TTR"discord.gg/Acrp");
			SendClientMessage(playerid, -1, ""TTR"[Server]: "WHITE"Selamat bermain dan memulai cerita di "TTR"Astral City Roleplay");
...
public OnPlayerEnterRaceCheckpoint(playerid)
{
	...
	if (gPoliceAlertCP[playerid])
	{
		...
		for(new i = 0; i < 13; i++)
		{
			PlayerTextDrawHide(playerid, PenembakanDeteksi[playerid][i]);
		}
...
public OnPlayerKeyStateChange(playerid, newkeys, oldkeys)
{
	...
	if (PRESSED(KEY_FIRE) || PRESSED(KEY_HANDBRAKE))
	{
		...
		if (weaponid >= 22 && weaponid <= 38 && GetPlayerState(playerid) == PLAYER_STATE_ONFOOT)
		{
			new slot = g_aWeaponSlots[weaponid];
			if (AccountData[playerid][pAmmo][slot] <= 0)
			{
				new clip_count = Inventory_Count(playerid, "Clip");

				if (clip_count > 0)
				{
					new max_add = 200 - AccountData[playerid][pAmmo][slot];
					new add_amount = clip_count;

					if (add_amount > max_add)
						add_amount = max_add;

					AccountData[playerid][pAmmo][slot] += add_amount;
					Inventory_Remove(playerid, "Clip", add_amount);

					PlayerTextDrawSetString(playerid, AMMOTD[playerid], sprintf("%d", AccountData[playerid][pAmmo][slot]));

					ApplyAnimation(playerid, "PYTHON", "python_reload", 4.0, 0, 0, 0, 0, 0);
					PlayerPlaySound(playerid, 36401, 0.0, 0.0, 0.0);
					ShowItemBox(playerid, sprintf("Removed %dx", add_amount), "Clip", 19995);
					ShowPlayerFooter(playerid, sprintf("Senjata diisi otomatis %d peluru dari inventory.", add_amount), 3000, true);
				}
				else
				{
					TogglePlayerControllable(playerid, 0);
    ...
    if (PRESSED(KEY_CTRL_BACK) && GetPlayerState(playerid) == PLAYER_STATE_ONFOOT)
    {
        ...
        {
            new slot = g_aWeaponSlots[weaponid];
            new ammo = AccountData[playerid][pAmmo][slot];

            if (ammo <= 0)
                return ShowTDN(playerid, NOTIFICATION_ERROR, "Tidak ada peluru di senjata ini.");

            Inventory_Add(playerid, "Clip", 19995, ammo);
            AccountData[playerid][pAmmo][slot] = 0;

            PlayerTextDrawSetString(playerid, AMMOTD[playerid], "0");
            SetPlayerArmedWeapon(playerid, 0);

			ShowItemBox(playerid, sprintf("Received %dx", ammo), "Clip", 19995);
            Info(playerid, "Kamu menyimpan %d peluru ke dalam inventory.", ammo);
        }
    }
	/* Senter */
    ...
    {
        if(!AccountData[playerid][IsLoggedIn]) return 1;
	...
	{
		if(AccountData[playerid][pInjured] > 0) return ShowTDN(playerid, NOTIFICATION_ERROR, "Tidak dapat membuka radial saat sedang keadaan Pingsan!");
	...
	if(newkeys & KEY_WALK && AccountData[playerid][pInjured])
	{
		...
		foreach(new i : Player) if (AccountData[i][pSpawned] && AccountData[i][pFaction] == FACTION_EMS) if (AccountData[i][pDutyEms])
		{
			...
			Syntax(i, "Gunakan "YELLOW"'Buka gps'"WHITE" untuk merespon sinyal");
		}
	}
	//-----[ Toll System ]-----	
...
public OnPlayerStateChange(playerid, newstate, oldstate)
{
    ...
    {	
		...
		for(new txd = 0; txd < 14; txd++) 
		{
			PlayerTextDrawHide(playerid, SPEEDONEW[playerid][txd]);
		}
	}
    ...
    {	
		...
		for(new txd = 0; txd < 14; txd++) 
		{
			PlayerTextDrawShow(playerid, SPEEDONEW[playerid][txd]);
		}
...
public OnPlayerTakeDamage(playerid, issuerid, Float:amount, weaponid, bodypart)
{
	...
	if (issuerid != INVALID_PLAYER_ID && bodypart == 3 && weaponid >= 22 && weaponid <= 45)
	{
		...
		new genderStr[16];
		format(genderStr, sizeof(genderStr), "%s", (AccountData[playerid][pGender] == 2) ? ("Perempuan") : ("Laki - Laki"));
		...
		foreach (new i : Player)
		{
			if (IsPlayerConnected(i) && SQL_IsCharacterLogged(i))
			{
				if (AccountData[i][pFaction] == FACTION_POLISI && AccountData[i][pDutyPD])
				{
					...
					gPoliceAlertCP[i] = true;
					
					for(new a = 0; a < 13; a++)
					{
						PlayerTextDrawSetString(i, PenembakanDeteksi[i][5], GetLocation(x, y, z));
						PlayerTextDrawSetString(i, PenembakanDeteksi[i][9], genderStr);
						PlayerTextDrawSetString(i, PenembakanDeteksi[i][12], weaponName);
						PlayerTextDrawShow(i, PenembakanDeteksi[i][a]);
					}
				}
			}
		}
...
Dialog:DIALOG_EJECT_SEAT(playerid, response, listitem, inputtext[])
{
    if(!response) return 1;

    new vehid = GetPlayerVehicleID(playerid);
    new seat = listitem + 1; // 1 = kursi2, 2 = kursi3, 3 = kursi4
    new bool:found = false;

    foreach(new otherid : Player)
    {
        if(otherid == playerid) continue; // jangan eject driver

        if(IsPlayerInVehicle(otherid, vehid))
        {
            if(GetPlayerVehicleSeat(otherid) == seat)
            {
				RemovePlayerFromVehicle(otherid);
                new Float:x, Float:y, Float:z;
                GetPlayerPos(playerid, x, y, z);
                PutPlayerInVehicle(otherid, vehid, 128);
                SetPlayerPos(otherid, x+1.0, y+3.0, z);
    ...
    if(!found)
    {
        ShowTDN(playerid, NOTIFICATION_ERROR, sprintf("Tidak ada penumpang di kursi %d!", seat+1));
    }
    return 1;
}

Dialog:Skin_InputBizz(playerid, response, listitem, inputtext[])
{
    if (!response) return 1;

    new index = strval(inputtext);
    if (index <= 0)
        return SendClientMessage(playerid, -1, "Masukkan angka yang valid!");

    new skinid = -1;

    if (AccountData[playerid][pGender] == 1) // Laki-laki
    {
        if (index > sizeof(ClothesSkinMale))
            return SendClientMessage(playerid, -1, "Nomor skin tidak valid! (1 - 177)");

        skinid = ClothesSkinMale[index - 1];
    }
    else if (AccountData[playerid][pGender] == 2) // Perempuan
    {
        if (index > sizeof(ClothesSkinFemale))
            return SendClientMessage(playerid, -1, "Nomor skin tidak valid! (1 - 60)");

        skinid = ClothesSkinFemale[index - 1];
    }

    if (skinid != -1)
    {
        SetPlayerSkin(playerid, skinid);
        CSelectBizz[playerid] = index - 1;
        BuyClothesBizz[playerid] = 1;

        new current = index;
        new total = (AccountData[playerid][pGender] == 1) ? sizeof(ClothesSkinMale) : sizeof(ClothesSkinFemale);

        new string[32];
        format(string, sizeof(string), "%d/%d", current, total);
        PlayerTextDrawSetString(playerid, ClothesMenuBizz[playerid][26], string);
    }
    return 1;
}

Dialog:Accessory_InputBizz(playerid, response, listitem, inputtext[])
{
    if (!response) return 1;
    ...
    if (index <= 0)
    {
        SendClientMessage(playerid, -1, "Masukkan angka yang valid!");
        return 1;
    }

    if (index > sizeof(AksesorisToys))
    {
        SendClientMessage(playerid, -1, "Nomor aksesoris tidak valid! (1 - 38)");
        return 1;
    }

    SelectAccBizz[playerid] = index - 1;
    SetPlayerAttachedObject(playerid, 9, AksesorisToys[SelectAccBizz[playerid]], 2, -0.392, 0.362, 0.000, 0.0, 0.0, 0.0, 1.0, 1.0, 1.0);
...
Dialog:Skin_Input(playerid, response, listitem, inputtext[])
{
    if (!response) return 1;
    ...
    if (skinid != -1)
    {
        ...
        new string[32];
        format(string, sizeof(string), "%d/%d", current, total);
        PlayerTextDrawSetString(playerid, ClotheMenu[playerid][26], string);
    }
    return 1;
}

Dialog:Accessory_Input(playerid, response, listitem, inputtext[])
{
    if (!response) return 1;
    ...
    SelectAcc[playerid] = index - 1;
    SetPlayerAttachedObject(playerid, 9, AksesorisToys[SelectAcc[playerid]], 2, -0.392, 0.362, 0.000, 0.0, 0.0, 0.0, 1.0, 1.0, 1.0);

    new minsty[128];
    format(minsty, sizeof(minsty), "%02d/%d", SelectAcc[playerid] + 1, sizeof(AksesorisToys));
    PlayerTextDrawSetString(playerid, ClotheMenu[playerid][26], minsty);
...
Dialog:Accessory_Hat(playerid, response, listitem, inputtext[])
{
    if (!response) return 1;
    ...
    if (index > sizeof(AksesorisHat))
    {
        SendClientMessage(playerid, -1, "Nomor Topi tidak valid! (1 - 87)");
        return 1;
    }
...
Dialog:Accessory_HatBizz(playerid, response, listitem, inputtext[])
{
    if (!response) return 1;
...
Dialog:Backpack_Input(playerid, response, listitem, inputtext[])
{
    if (!response) return 1;
    ...
    if (index > sizeof(BackpackToys))
    {
        SendClientMessage(playerid, -1, "Nomor Topi tidak valid! (1 - 7)");
        return 1;
    }
...
Dialog:Backpack_Bizz(playerid, response, listitem, inputtext[])
{
    if (!response) return 1;
...
Dialog:Glasses_Input(playerid, response, listitem, inputtext[])
{
    if (!response) return 1;
    ...
    if (index > sizeof(GlassesToys))
    {
        SendClientMessage(playerid, -1, "Nomor Kacamata tidak valid! (1 - 33)");
        return 1;
    }
...
Dialog:Glasses_Bizz(playerid, response, listitem, inputtext[])
{
    if (!response) return 1;
	...
	switch(listitem)
	{
		...
		case 8: //lihat sku
		{
			...
			mysql_format(g_SQL, query, sizeof(query),
				"SELECT graduate_name, degree, university, created_at, given_by \
				FROM graduation_letters WHERE owner_id = %d ORDER BY id DESC LIMIT 1",
				AccountData[playerid][pID]);

			mysql_tquery(g_SQL, query, "OnLoadShowSurat", "d", playerid);
		}
		case 9: //tunjuk sku
		{
			new query[256];
			...
			mysql_tquery(g_SQL, query, "OnLoadSuratShowNearby", "d", playerid);
		}
	}
	return 1;
}
...
{
	if(clickedid == TollTD[19])
    {
        new i = pCurrentBarrier[playerid];
        if (i != -1 && i < sizeof(BarrierInfo))
        {
            if (BarrierInfo[i][brOrg] == TEAM_NONE && !BarrierInfo[i][brOpen])
            {
                ...
                {
                    if (IsVehicleFaction(GetPlayerVehicleID(playerid)))
                    {
            ...
            {
                ShowTDN(playerid, NOTIFICATION_ERROR, "Anda tidak dapat membuka toll ini!");
            }
        }

        HideTollTD(playerid);
        pCurrentBarrier[playerid] = -1; // reset
    }
	return 1;
}
...
{
	if(playertextid == TextdrawLoginUI[playerid][20])
	{
		 if(GetPVarInt(playerid, "PasswordVerified"))
        {
            DeletePVar(playerid, "PasswordVerified");
			...
			for (new i = 0; i < 36; i++)
			{
				PlayerTextDrawHide(playerid, TextdrawLoginUI[playerid][i]);
			}
			CancelSelectTextDraw(playerid);
        }
	}
	if(playertextid == TextdrawLoginUI[playerid][25])
	{
		new frmxtdialog[255];
        format(frmxtdialog, sizeof(frmxtdialog), ""WHITE"Selamat datang di "TTR"Astral City Roleplay\n"WHITE"UCP Ini telah terdaftar!\nNama UCP: "LIGHTGREEN"%s\
		\n"WHITE"Version: "TTR"ACRP v2.1.1\n"YELLOW"(Silahkan masukkan kata sandi anda dengan benar untuk login):", AccountData[playerid][pUCP]);
		ShowPlayerDialog(playerid, DIALOG_LOGIN, DIALOG_STYLE_PASSWORD, "UCP - Login", frmxtdialog, "Input", "Keluar");
	}
	if(playertextid == HealthTD[playerid][46])
	{
		for (new i = 0; i < 48; i++)
		{
			PlayerTextDrawHide(playerid, HealthTD[playerid][i]);
		}
		CancelSelectTextDraw(playerid);
	}
	//Clothes Warung
	if(playertextid == ClotheMenu[playerid][4]) // Pakaian
	{
		if(BuyClothes[playerid] == 1)
		{
			SetCameraClothesWarung(playerid);
		...
		PlayerTextDrawShow(playerid, ClotheMenu[playerid][14]);
		PlayerTextDrawHide(playerid, ClotheMenu[playerid][15]);
		PlayerTextDrawHide(playerid, ClotheMenu[playerid][16]);
		PlayerTextDrawHide(playerid, ClotheMenu[playerid][17]);
		PlayerTextDrawHide(playerid, ClotheMenu[playerid][0]);
		BuyClothes[playerid] = 1;
		CSelect[playerid] = 0;
	...
	if(playertextid == ClotheMenu[playerid][8]) // Topi Dan Helmet
	{
		if(BuyClothes[playerid] == 1)
		{
			SetCameraClothesWarung(playerid);
		...
		PlayerTextDrawHide(playerid, ClotheMenu[playerid][14]);
		PlayerTextDrawHide(playerid, ClotheMenu[playerid][15]);
		PlayerTextDrawHide(playerid, ClotheMenu[playerid][16]);
		PlayerTextDrawHide(playerid, ClotheMenu[playerid][17]);
		PlayerTextDrawShow(playerid, ClotheMenu[playerid][0]);
		BuyTopi[playerid] = 1;
		SelectAcc[playerid] = 0;
	...
	if(playertextid == ClotheMenu[playerid][6]) // Kacamata Toys
	{
		if(BuyClothes[playerid] == 1)
		{
			SetCameraClothesWarung(playerid);
		...
		PlayerTextDrawHide(playerid, ClotheMenu[playerid][14]);
		PlayerTextDrawHide(playerid, ClotheMenu[playerid][15]);
		PlayerTextDrawShow(playerid, ClotheMenu[playerid][16]);
		PlayerTextDrawHide(playerid, ClotheMenu[playerid][17]);
		PlayerTextDrawHide(playerid, ClotheMenu[playerid][0]);
		BuyGlasses[playerid] = 1;
		SelectAcc[playerid] = 0;
	...
	if(playertextid == ClotheMenu[playerid][5]) // Aksesoris
	{
		if(BuyClothes[playerid] == 1)
		{
			SetCameraClothesWarung(playerid);
		...
		PlayerTextDrawHide(playerid, ClotheMenu[playerid][14]);
		PlayerTextDrawShow(playerid, ClotheMenu[playerid][15]);
		PlayerTextDrawHide(playerid, ClotheMenu[playerid][16]);
		PlayerTextDrawHide(playerid, ClotheMenu[playerid][17]);
		PlayerTextDrawHide(playerid, ClotheMenu[playerid][0]);
		BuyTAksesoris[playerid] = 1;
	...
	if(playertextid == ClotheMenu[playerid][7]) // Tas / Backpack
	{
		if(BuyClothes[playerid] == 1)
		{
			SetCameraClothesWarung(playerid);
		...
		PlayerTextDrawHide(playerid, ClotheMenu[playerid][15]);
		PlayerTextDrawHide(playerid, ClotheMenu[playerid][16]);
		PlayerTextDrawShow(playerid, ClotheMenu[playerid][17]);
		PlayerTextDrawHide(playerid, ClotheMenu[playerid][0]);
		BuyBackpack[playerid] = 1;
		SelectAcc[playerid] = 0;
	...
	if(playertextid == ClotheMenu[playerid][19]) // input skin
	{
		if(BuyClothes[playerid] == 1)
		{
			...
			Dialog_Show(playerid, Skin_Input, DIALOG_STYLE_INPUT, judul, isi, "Pilih", "Batal");
		}
		if(BuyTAksesoris[playerid] == 1)
		{
			new judul[64];
			...
			Dialog_Show(playerid, Accessory_Input, DIALOG_STYLE_INPUT, judul, isi, "Pilih", "Batal");
		}
		if(BuyTopi[playerid] == 1)
		{
			new judul[64];
			...
			Dialog_Show(playerid, Accessory_Hat, DIALOG_STYLE_INPUT, judul, isi, "Pilih", "Batal");
		}
		if(BuyBackpack[playerid] == 1)
		{
			new judul[64];
			...
			Dialog_Show(playerid, Backpack_Input, DIALOG_STYLE_INPUT, judul, isi, "Pilih", "Batal");
		}
		if(BuyGlasses[playerid] == 1)
		{
			new judul[64];
			...
			Dialog_Show(playerid, Glasses_Input, DIALOG_STYLE_INPUT, judul, isi, "Pilih", "Batal");
		}
	}
	if(playertextid == ClotheMenu[playerid][24]) // Batal
	{
		if(BuyClothes[playerid] == 1)
		{
			SetCameraClothesWarung(playerid);
		...
		{
			PlayerTextDrawHide(playerid, ClotheMenu[playerid][txd]);
		}
		Toggle_AllTextdraws(playerid, true);
		PlayerPlaySound(playerid, 1053, 0, 0, 0);
		CancelSelectTextDraw(playerid);
	}
	if(playertextid == ClotheMenu[playerid][21]) // Next Cloth
	{
		if(BuyClothes[playerid] == 1)
		{
	...
	if(playertextid == ClotheMenu[playerid][20]) // Prev Cloth
	{
		if(BuyClothes[playerid] == 1)
		{
	...
	if(playertextid == ClotheMenu[playerid][22]) // Rot Kiri
	{
	...
	if(playertextid == ClotheMenu[playerid][23]) // Rot Kanana
	{
		static Float:x, Float:y, Float:z, Float:angle;
		GetPlayerPos(playerid, x, y, z);
		GetPlayerFacingAngle(playerid, angle);
	...
	if(playertextid == ClotheMenu[playerid][18]) // Beli Clothes
	{
		if(BuyClothes[playerid] == 1)
		{
			new price = 200;
	...
	// Inventory
	if(playertextid == InventoryTD[playerid][5])
	{
		Inventory_Close(playerid);
	}
		...
		if(playertextid == BoxInv[playerid][x])
		{
			...
			if(item_warung == -1) {
				...
				{	
					...
					{
						PlayerTextDrawColor(playerid, BoxInv[playerid][AccountData[playerid][pSelectItem]], 0x3399FF66); // warna normal
						...
						PlayerTextDrawColor(playerid, BoxInv[playerid][x], 0x3399FF66);
						PlayerTextDrawColor(playerid, BoxInv[playerid][selectedid], 0x3399FF66);
						...
						PlayerTextDrawColor(playerid, BoxInv[playerid][x], 0x8B636CAA);
					...
					{
						PlayerTextDrawColor(playerid, BoxInv2[playerid][AccountData[playerid][pSelectItem]], 0x3399FF66);
				...
				else 
				{
					...
					{
						new name[128];
						new fromSlot = AccountData[playerid][pSelectItem];
						if(fromSlot != -1 && InventoryData[playerid][fromSlot][invExists])
						{
							strunpack(name, InventoryData[playerid][fromSlot][invItem]);

							new model = InventoryData[playerid][fromSlot][invModel];
							new amount = InventoryData[playerid][fromSlot][invQuantity];
							new itemid = InventoryData[playerid][fromSlot][invID];

							new invstr[1024];
							mysql_format(g_SQL, invstr, sizeof(invstr), "DELETE FROM `inventory` WHERE `ID` = %d AND `invID` = %d", AccountData[playerid][pID], itemid);
							...
							InventoryData[playerid][x][invModel] = model;
							InventoryData[playerid][x][invQuantity] = amount;
							InventoryData[playerid][x][invID] = itemid;
							strcpy(InventoryData[playerid][x][invItem], name);

							PlayerTextDrawSetPreviewModel(playerid, PrevMod[playerid][x], model);
							PlayerTextDrawShow(playerid, PrevMod[playerid][x]);

							PlayerTextDrawSetString(playerid, NameInv[playerid][x], sprintf("%s", name));
							PlayerTextDrawShow(playerid, NameInv[playerid][x]);

							PlayerTextDrawSetString(playerid, QuantityInv[playerid][x], sprintf("%dx", amount));
							...
							InventoryData[playerid][fromSlot][invExists] = false;
							InventoryData[playerid][fromSlot][invModel] = 0;
							InventoryData[playerid][fromSlot][invQuantity] = 0;
							InventoryData[playerid][fromSlot][invID] = 0;
							InventoryData[playerid][fromSlot][invItem][0] = EOS;

							PlayerTextDrawHide(playerid, PrevMod[playerid][fromSlot]);
							PlayerTextDrawHide(playerid, NameInv[playerid][fromSlot]);
							PlayerTextDrawHide(playerid, QuantityInv[playerid][fromSlot]);
							PlayerTextDrawHide(playerid, LineInv[playerid][fromSlot]);
							PlayerTextDrawColor(playerid, BoxInv[playerid][fromSlot], 0x3399FF66);
							PlayerTextDrawShow(playerid, BoxInv[playerid][fromSlot]);

							mysql_format(g_SQL, invstr, sizeof(invstr), 
							"INSERT INTO `inventory` (`ID`, `invItem`, `invModel`, `invQuantity`) VALUES('%d', '%e', '%d', '%d')",
							AccountData[playerid][pID], name, model, amount);
						...
						if(AccountData[playerid][pStorageSelect] == 0) 
						{
							...
							if(id != -1) 
							{
								...
								{	
									if(AccountData[playerid][pSelectItemDrop] != -1)
									{
										PlayerTextDrawColor(playerid, BoxInv2[playerid][AccountData[playerid][pSelectItemDrop]], 0x3399FF66);
										PlayerTextDrawShow(playerid, BoxInv2[playerid][AccountData[playerid][pSelectItemDrop]]);
									}
									RemoveDroppedItem(id, amount);
									Inventory_Refresh(playerid);
				...
				if(!strcmp(WarungItemName[playerid][item_warung], "Rokok")) {
					...
					if(selectItemWarung[playerid] != -1)
					{
						PlayerTextDrawColor(playerid, BoxInv2[playerid][selectItemWarung[playerid]], 0x3399FF66);
						PlayerTextDrawShow(playerid, BoxInv2[playerid][selectItemWarung[playerid]]);
					}
					return 1;
				}
			...
			switch (AccountData[playerid][pStorageSelect])
			{
				case 0:
				{
					...
					else 
					{
						...
						PlayerTextDrawColor(playerid, BoxInv2[playerid][x], 0x8B636CAA);
						PlayerTextDrawShow(playerid, BoxInv2[playerid][x]);
					}
				}
				case 1:
				{
					if(selectItemWarung[playerid] != -1) {
						PlayerTextDrawColor(playerid, BoxInv2[playerid][selectItemWarung[playerid]], 0x3399FF66);
						PlayerTextDrawHide(playerid, BoxInv2[playerid][selectItemWarung[playerid]]);
						PlayerTextDrawShow(playerid, BoxInv2[playerid][selectItemWarung[playerid]]);
					}
					...
					PlayerTextDrawColor(playerid, BoxInv2[playerid][x], 0x8B636CAA);
					PlayerTextDrawHide(playerid, BoxInv2[playerid][x]);
					PlayerTextDrawShow(playerid, BoxInv2[playerid][x]);
	...
	if(playertextid == InventoryTD[playerid][2])
	{
		...
		ShowPlayerDialog(playerid, DIALOG_SETAMOUNT, DIALOG_STYLE_INPUT, ""TTR"Astral City Roleplay "WHITE"- Set Amount",
		"Mohon masukkan berapa jumlah item yang akan diberikan:", "Set", "Batal");
	}
	if(playertextid == InventoryTD[playerid][3])
	{
		new
	...
	if(playertextid == InventoryTD[playerid][4])
	{
		if(AccountData[playerid][pSelectItem] == -1)
		...
		if(count == 0)
		{
			...
			return ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""TTR"Astral City Roleplay "WHITE"- Give Item",
			"Tidak ada player yang dekat dengan anda!", "Tutup", "");
		}
		ShowPlayerDialog(playerid, DIALOG_MEMBERI, DIALOG_STYLE_LIST, ""TTR"Astral City Roleplay "WHITE"- Give Item", frmxt, "Pilih", "Close");
	}
	return 1;
}
