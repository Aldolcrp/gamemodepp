...
GivePlayerInvoice(playerid, targetid)
{
    ...
    ShowPlayerDialog(playerid, DIALOG_INVOICE_NAME, DIALOG_STYLE_INPUT, ""TTR"Astral City Roleplay "WHITE"- Invoice", "Mohon masukkan nama untuk tagihan ini:", "Input", "Batal");
    return 1;
}
...
PeriksaInvoice(playerid, targetid)
{
    ...
    if(cache_num_rows() > 0)
    {
        ...
        ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""TTR"Astral City Roleplay "WHITE"- Invoice Pending", affah, "Tutup", "");
    }
    else
        ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""TTR"Astral City Roleplay "WHITE"- Invoice Pending", "Pemain tersebut tidak memiliki Tagihan.", "Tutup", "");
...
ShowPlayerInvoice(playerid)
{
    ...
    if(rows)
    {
        ...
        ShowPlayerDialog(playerid, DIALOG_PAY_INVOICE, DIALOG_STYLE_TABLIST_HEADERS, ""TTR"Astral City Roleplay "WHITE"- Invoice Pending", affah, "Pilih", "Batal");
    }
    else
    {
        ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""TTR"Astral City Roleplay "WHITE"- Invoice belum terbayar", 
...
hook OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    if(dialogid == DIALOG_INVOICE_NAME)
    {
        ...
        if(isnull(inputtext)) return ShowPlayerDialog(playerid, DIALOG_INVOICE_NAME, DIALOG_STYLE_INPUT, ""TTR"Astral City Roleplay "WHITE"- Invoice", 
        "Error: Nama tidak boleh kosong!\nMohon masukkan nama untuk tagihan ini:", "Input", "Batal");

        if(IsNumeric(inputtext)) return ShowPlayerDialog(playerid, DIALOG_INVOICE_NAME, DIALOG_STYLE_INPUT, ""TTR"Astral City Roleplay "WHITE"- Invoice", 
        "Error: Harus berformat nama, tidak boleh angka!\nMohon masukkan nama untuk tagihan ini:", "Input", "Batal");

        if(strlen(inputtext) < 1 || strlen(inputtext) > 128) return ShowPlayerDialog(playerid, DIALOG_INVOICE_NAME, DIALOG_STYLE_INPUT, ""TTR"Astral City Roleplay "WHITE"- Invoice", 
        "Error: Nama tidak dapat kurang dari 1 huruf atau lebih dari 125!\nMohon masukkan nama untuk tagihan ini:", "Input", "Batal");

        if(!IsValidFormatText(inputtext)) return ShowPlayerDialog(playerid, DIALOG_INVOICE_NAME, DIALOG_STYLE_INPUT, ""TTR"Astral City Roleplay "WHITE"- Invoice", 
        "Error: Tidak dapat menggunakan tanda kutip ' '\nMohon masukkan nama untuk tagihan ini:", "Input", "Batal");

        SetPVarString(playerid, "InvoiceName", inputtext);
        ShowPlayerDialog(playerid, DIALOG_INVOICE_COST, DIALOG_STYLE_INPUT, ""TTR"Astral City Roleplay "WHITE"- Invoice", "Mohon masukkan nominal tagihan dikolom bawah ini:", "Input", "Batal");
    }
    else if(dialogid == DIALOG_INVOICE_COST)
    {
        ...
        if(isnull(inputtext)) return ShowPlayerDialog(playerid, DIALOG_INVOICE_COST, DIALOG_STYLE_INPUT, ""TTR"Astral City Roleplay "WHITE"- Invoice", 
        "Error: Nominal tagihan tidak boleh kosong\nMohon masukkan nominal tagihan dikolom bawah ini:", "Input", "Batal");

        if(!IsNumeric(inputtext)) return ShowPlayerDialog(playerid, DIALOG_INVOICE_COST, DIALOG_STYLE_INPUT, ""TTR"Astral City Roleplay "WHITE"- Invoice", 
        "Error: Hanya dapat diisi angka!\nMohon masukkan nominal tagihan dikolom bawah ini:", "Input", "Batal");

        if(strval(inputtext) < 1) return ShowPlayerDialog(playerid, DIALOG_INVOICE_COST, DIALOG_STYLE_INPUT, ""TTR"Astral City Roleplay "WHITE"- Invoice", 
