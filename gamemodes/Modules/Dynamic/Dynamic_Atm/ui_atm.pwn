...
{
	if(playertextid == IDPATM[playerid][6])// Withdraw
	{
		ShowPlayerDialog(playerid, DIALOG_ATM_WITHDRAW, DIALOG_STYLE_INPUT, ""TTR"Astral City Roleplay "WHITE"- Fleeca Bank", "Mohon masukan berapa jumlah uang yang anda ingin anda ambil:", "Submit", "Batal");
	}
	if(playertextid == IDPATM[playerid][5])// Deposit
	{
		ShowPlayerDialog(playerid, DIALOG_ATM_DEPOSIT, DIALOG_STYLE_INPUT, ""TTR"Astral City Roleplay "WHITE"- Fleeca Bank", "Mohon masukan berapa jumlah uang yang ingin anda masukkan:", "Submit", "Batal");
	}
	if(playertextid == IDPATM[playerid][7])// Transfer
	{
		ShowPlayerDialog(playerid, DIALOG_ATM_TRANSFER, DIALOG_STYLE_INPUT, ""TTR"Astral City Roleplay "WHITE"- Fleeca Bank", "Mohon masukkan nomor rekening yang ingin anda transfer:", "Submit", "Batal");
	}
	if(playertextid == IDPATM[playerid][1])// Log Out
	{
		HideATMTD(playerid);
	}
	return 1;
...
hook OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
	switch(dialogid)
	{
		case DIALOG_ATM_TRANSFER:
		{
			...
			if(isnull(inputtext)) return ShowPlayerDialog(playerid, DIALOG_ATM_TRANSFER, DIALOG_STYLE_INPUT, ""TTR"Astral City Roleplay "WHITE"- Fleeca Bank", 
			"Error: Harus diisi tidak dapat kosong!\nMohon masukkan nomor rekening yang ingin anda transfer:", "Submit", "Batal");

			if(!IsNumeric(inputtext)) return ShowPlayerDialog(playerid, DIALOG_ATM_TRANSFER, DIALOG_STYLE_INPUT, ""TTR"Astral City Roleplay "WHITE"- Fleeca Bank", 
		...
		case DIALOG_ATM_TRANSFER1:
		{
			...
			if(isnull(inputtext)) 
			{
				return ShowPlayerDialog(playerid, DIALOG_ATM_TRANSFER1, DIALOG_STYLE_INPUT, ""TTR"Astral City Roleplay "WHITE"- Fleeca Bank", 
		...
		case DIALOG_ATM_DEPOSIT:
		{
			...
			if(isnull(inputtext))
			{
				ShowPlayerDialog(playerid, DIALOG_ATM_DEPOSIT, DIALOG_STYLE_INPUT, ""TTR"Astral City Roleplay "WHITE"- Fleeca Bank", 
			...
			PlayerTextDrawSetString(playerid, IDPATM[playerid][15], sprintf("%s", FormatMoney(AccountData[playerid][pBankMoney])));
			PlayerTextDrawShow(playerid, IDPATM[playerid][15]);
		...
		case DIALOG_ATM_WITHDRAW:
		{
			...
			if(isnull(inputtext))
			{
				ShowPlayerDialog(playerid, DIALOG_ATM_WITHDRAW, DIALOG_STYLE_INPUT, ""TTR"Astral City Roleplay "WHITE"- Fleeca Bank", 
...
FUNC::SearchRekening(playerid, norek)
{
	...
	else 
	{
		...
		ShowPlayerDialog(playerid, DIALOG_ATM_TRANSFER1, DIALOG_STYLE_INPUT, ""TTR"Astral City Roleplay "WHITE"- Fleeca Bank", "Mohon masukkan nominal uang yang ingin anda transfer:", "Submit", "Batal");
	}
	return 1;
}
