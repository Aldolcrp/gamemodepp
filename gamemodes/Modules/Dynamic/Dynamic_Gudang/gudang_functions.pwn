        ...
        if(id > -1)
        {
            if(IsPlayerInRangeOfPoint(playerid, 2.0, GudangData[id][gudangPOS][0], GudangData[id][gudangPOS][1], GudangData[id][gudangPOS][2]))
            {
                if(AccountData[playerid][pHasGudangID] == id)
                {
                    ...
                    ShowPlayerDialog(playerid, DIALOG_GUDANG_OPTION, DIALOG_STYLE_LIST, ""TTR"Astral City Roleplay "WHITE"- Gudang",
                ...
                else 
                {
                    ...
                    AccountData[playerid][pTempValue] = id;
                    ShowPlayerDialog(playerid, DIALOG_GUDANG_BUY, DIALOG_STYLE_LIST, sprintf(""TTR"Astral City Roleplay "WHITE"- Gudang %s", GudangData[id][gudangName]),
                    shstr, "Pilih", "Batal");
                }
            }
        }
...
hook OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    ...
    else if(dialogid == DIALOG_GUDANG_OPTION)
    {
        ...
        switch(listitem)
        {
            ...
            {
                AccountData[playerid][menuShowed] = true;
                ShowPlayerDialog(playerid, DIALOG_GUDANGVAULT, DIALOG_STYLE_LIST, ""TTR"Astral City Roleplay "WHITE"- Gudang",
                "Simpan Barang\n"GRAY"Ambil Barang", "Pilih", "Batal");
            }
            case 1: //berhenti sewa
            {
                Dialog_Show(playerid, GUDANG_STOPCONF, DIALOG_STYLE_MSGBOX, ""TTR"Astral City Roleplay", 
            ...
            case 2: //status sewa
            {
                ...
                if(gdid != -1)
                {
                    ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, sprintf(""TTR"Astral City Roleplay "WHITE"- %s", GudangData[gdid][gudangName]),
                    shstr, "Tutup", "");
                }
            }
        }
        ...
        switch(listitem)
        {
            case 0: //deposit
            {
                ...
                if(rows)
                {
                    ...
                    ShowPlayerDialog(playerid, DIALOG_GUDANGVAULT_DEPOSIT, DIALOG_STYLE_TABLIST_HEADERS, ""TTR"Astral City Roleplay "WHITE"- Gudang", str, "Pilih", "Batal");
                }
                else
                {
                    PlayerPlaySound(playerid, 5206, 0.0, 0.0, 0.0);
                    return ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""TTR"Astral City Roleplay "WHITE"- Gudang",
            ...
            case 1: //withdraw
            {
                ...
                if(cache_num_rows() > 0)
                {
                    ...
                    ShowPlayerDialog(playerid, DIALOG_GUDANGVAULT_WITHDRAW, DIALOG_STYLE_TABLIST_HEADERS, ""TTR"Astral City Roleplay "WHITE"- Gudang", shstr, "Pilih", "Batal");
                }
                else 
                {
                    return ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""TTR"Astral City Roleplay "WHITE"- Gudang",
    ...
    else if(dialogid == DIALOG_GUDANGVAULT_DEPOSIT)
    {
        ...
        if(cache_num_rows() > 0)
        {
            ...
            ShowPlayerDialog(playerid, DIALOG_GUDANGVAULT_IN, DIALOG_STYLE_INPUT, ""TTR"Astral City Roleplay "WHITE"- Gudang", shstr, "Input", "Batal");
        }
    }
    else if(dialogid == DIALOG_GUDANGVAULT_IN)
    {
        ...
        if(isnull(inputtext) || !IsNumeric(inputtext))
        {
            ...
            format(shstr, sizeof(shstr), "Anda akan menyimpan item:\nNama: %s\nJumlah di tas: %d\nMohon masukkan jumlah valid!",
                GudangBrankas[playerid][gudangBrankasTemp], GudangBrankas[playerid][gudangBrankasQuant]);
            ShowPlayerDialog(playerid, DIALOG_GUDANGVAULT_IN, DIALOG_STYLE_INPUT, "TTR Astral City Roleplay - Gudang", 
                shstr, "Input", "Batal");
            GudangProcessing[playerid] = false;
            return 1;
        }

        new requested = strval(inputtext);
        new avail = Inventory_Count(playerid, GudangBrankas[playerid][gudangBrankasTemp]);
        ...
        if(GetTotalWeightGudang(playerid) + requested > 500)
        {
            GudangProcessing[playerid] = false;
            AccountData[playerid][menuShowed] = false;
            return ShowTDN(playerid, NOTIFICATION_ERROR, "Kapasitas gudang anda telah penuh!");
        }

        Inventory_Remove(playerid, GudangBrankas[playerid][gudangBrankasTemp], requested);
        AddCapacityGudang(playerid, GudangBrankas[playerid][gudangBrankasTemp], requested);
        ShowItemBox(playerid, sprintf("Removed %dx", requested), GudangBrankas[playerid][gudangBrankasTemp], GudangBrankas[playerid][gudangBrankasModel]);

        new invstr[1028];
        mysql_format(g_SQL, invstr, sizeof(invstr),
            "INSERT INTO `player_gudang` (`ID`,`itemName`,`itemModel`,`itemQuantity`) VALUES(%d, '%e', %d, %d) ON DUPLICATE KEY UPDATE `itemQuantity` = `itemQuantity` + %d",
            AccountData[playerid][pID],
            GudangBrankas[playerid][gudangBrankasTemp],
            GudangBrankas[playerid][gudangBrankasModel],
            requested,
            requested);
    ...
    else if(dialogid == DIALOG_GUDANGVAULT_WITHDRAW)
    {
        ...
        if(cache_num_rows() > 0)
        {
            ...
            ShowPlayerDialog(playerid, DIALOG_GUDANGVAULT_OUT, DIALOG_STYLE_INPUT, ""TTR"Astral City Roleplay "WHITE"- Gudang",
            shstr, "Input", "Batal");
        }
    }
...
Dialog:GUDANG_STOPCONF(playerid, response, listitem, inputtext[])
{
    ...
    {
        return ShowPlayerDialog(playerid, DIALOG_GUDANG_OPTION, DIALOG_STYLE_LIST, ""TTR"Astral City Roleplay "WHITE"- Gudang",
        "Buka Gudang\n"GRAY"Berhenti menyewa gudang\nCek status sewa", "Pilih", "Batal");
    }
    return 1;
}
