...
ShowHouseDialog(playerid, hid)
{
    ...
    if(HouseData[hid][hsType] > 3)
    {
        ...
        ShowPlayerDialog(playerid, DIALOG_HOUSE_BRANKAS, DIALOG_STYLE_LIST, sprintf(""TTR"Astral City Roleplay "WHITE"- %s", HouseData[hid][hsOwner]),
        string, "Pilih", "Batal");
    }
    else
    {
...
ShowKeysHouseList(playerid)
{
    ...
    ShowPlayerDialog(playerid, DIALOG_HKEYS, DIALOG_STYLE_LIST, ""TTR"Astral City Roleplay "WHITE"- Pemegang Kunci", playerName, "Pilih", "Batal");
    return 1;
}
...
House_Typee(houseid)
{
    ...
    {
        HouseData[houseid][hsIntPos][0] = 1226.925;
        HouseData[houseid][hsIntPos][1] = -1386.688;
        HouseData[houseid][hsIntPos][2] = 813.534;
        HouseData[houseid][hsIntPos][3] = 262.482;
        HouseData[houseid][hsInt] = 35;
    }
    new query[512];
...
CMD:edithouse(playerid, params[])
{
    ...
    else if(!strcmp(type, "owner", true))
    {
        ...
        if(sscanf(string, "u", otherid)) 
            return Syntax(playerid, "/edithouse [id] [owner] [name/playerid]");
        if(!IsPlayerConnected(otherid)) 
            return Error(playerid, "Pemain tersebut tidak terkoneksi ke server!");

        new owned_houses = 0;
        foreach(new ii : House)
        {
            if(HouseData[ii][hsExtPos][0] != 0.0 && HouseData[ii][hsOwnerID] == AccountData[otherid][pID])
                owned_houses++;
        }

        new max_slots = AccountData[otherid][pHouseSlotPlus];
        if(max_slots < 1) max_slots = 1;

        if(owned_houses >= max_slots)
            return Error(playerid, "Pemain tersebut sudah mencapai batas maksimal kepemilikan rumah! (Maks: %d)", max_slots);

        static oldhousename[MAX_PLAYER_NAME];
        format(oldhousename, sizeof(oldhousename), "%s", HouseData[did][hsOwner]);

        format(HouseData[did][hsOwner], MAX_PLAYER_NAME, "%s", AccountData[otherid][pName]);
        HouseData[did][hsOwnerID] = AccountData[otherid][pID];

        HouseRefresh(did);
        HouseSave(did);

        SendStaffMessage(X11_TOMATO, "%s telah mengubah hak milik Rumah %d dari %s menjadi milik %s", AccountData[playerid][pAdminname], did, oldhousename, AccountData[otherid][pName]);

        static shstr[200];
        format(shstr, sizeof(shstr), "Menggunakan cmd /edithouse [owner] pada House ID %d: milik %s menjadi milik %s.", 
            did, oldhousename, AccountData[otherid][pName]);
...
stock GetHouseName(hid, dest[], len)
{
    if (hid == -1)
    {
    ...
    new type = HouseData[hid][hsType];

    if (type == 1)
        format(dest, len, "Standart");
    else if (type == 2)
        format(dest, len, "Medium");
    else if (type == 3)
        format(dest, len, "Modern");
    else if (type == 4 || type == 99)
        format(dest, len, "Mansion");
    else
        format(dest, len, "Unknown");
...
ShowHouseWeapons(playerid, hid) 
{
    ...
    ShowPlayerDialog(playerid, DIALOG_WEAPON_CHEST, DIALOG_STYLE_TABLIST_HEADERS, ""TTR"Astral City Roleplay "WHITE"- Weapon Chest", shstr, "Pilih", "Batal");
    return 1;
}
    ...
    if(newkeys & KEY_YES && GetPlayerState(playerid) == PLAYER_STATE_ONFOOT)
    {
        ...
        if(hid > -1)
        {
            if(IsPlayerInRangeOfPoint(playerid, 1.0, HouseData[hid][hsExtPos][0], HouseData[hid][hsExtPos][1], HouseData[hid][hsExtPos][2]))
            {
                ...
                {
                    new title[100];
                    format(title, sizeof(title), ""TTR"Astral City Roleplay "WHITE"- %s", HouseData[hid][hsOwner]);
...
hook OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    switch(dialogid)
    {
        case DIALOG_HOUSELIST:
        {
            if(!response) return 1;

            new hid;
            // Ambil House ID dari baris yang dipilih
            ...
            SetPlayerRaceCheckpoint(playerid, 1,
                ...
                HouseData[hid][hsExtPos][0],
                HouseData[hid][hsExtPos][1],
                HouseData[hid][hsExtPos][2], 4.0);

            ShowTDN(playerid, NOTIFICATION_INFO, "Lokasi rumah telah ditandai di map!");
        }
            ...
            switch(listitem)
            {
                case 0: //deposit
                {
                    ...
                    if(rows)
                    {
                        ...
                        ShowPlayerDialog(playerid, DIALOG_HOUSEVAULT_DEPOSIT, DIALOG_STYLE_TABLIST_HEADERS, ""TTR"Astral City Roleplay "WHITE"- Brankas Rumah", str, "Pilih", "Batal");
                    }
                    else 
                    {
                        PlayerPlaySound(playerid, 5206, 0.0, 0.0, 0.0);
                        return ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""TTR"Astral City Roleplay "WHITE"- Brankas Rumah",
                ...
                case 1: //withdraw
                {
                    ...
                    if(cache_num_rows())
                    {
                        ...
                        ShowPlayerDialog(playerid, DIALOG_HOUSEVAULT_WITHDRAW, DIALOG_STYLE_TABLIST_HEADERS, ""TTR"Astral City Roleplay "WHITE"- Brankas Rumah", str, "Pilih", "Batal");
                    }
                    else 
                    {
                        PlayerPlaySound(playerid, 5206, 0.0, 0.0, 0.0);
        ...
        case DIALOG_HOUSEVAULT_DEPOSIT:
        {
            ...
            if(cache_num_rows() > 0)
            {
                ...
                ShowPlayerDialog(playerid, DIALOG_HOUSEVAULT_IN, DIALOG_STYLE_INPUT, ""TTR"Astral City Roleplay "WHITE"- Brankas Rumah", 
                shstr, "Input", "Batal");
            }
        }
        case DIALOG_HOUSEVAULT_IN:
        {
            ...
            // LOCK player untuk mencegah double
            if(HouseProcessing[playerid])
                return ShowTDN(playerid, NOTIFICATION_WARNING, "Sedang memproses permintaan, tunggu sebentar!");
            HouseProcessing[playerid] = true;
            ...
            {
                format(shstr, sizeof(shstr),
                    "Anda akan menyimpan item:\nNama: %s\nJumlah di tas: %d\nMohon masukkan jumlah valid!",
                    HouseBrankas[playerid][housebrankasTemp], HouseBrankas[playerid][housebrankasQuant]);
                ShowPlayerDialog(playerid, DIALOG_HOUSEVAULT_IN, DIALOG_STYLE_INPUT,
                    "TTR Astral City Roleplay - Brankas Rumah", shstr, "Input", "Batal");
                HouseProcessing[playerid] = false;
                return 1;
            }
            ...
            new avail = Inventory_Count(playerid, HouseBrankas[playerid][housebrankasTemp]);
            if(avail <= 0)
            {
                HouseProcessing[playerid] = false;
                AccountData[playerid][menuShowed] = false;
            ...
            // cap jumlah requested sesuai stok
            if(requested < 1) requested = 1;
            if(requested > avail) requested = avail;

            // hapus item dari inventory server-side (atomic)
            Inventory_Remove(playerid, HouseBrankas[playerid][housebrankasTemp], requested);
            ShowItemBox(playerid, sprintf("Removed %dx", requested), HouseBrankas[playerid][housebrankasTemp], HouseBrankas[playerid][housebrankasModel]);
            AddCapacityHouse(AccountData[playerid][pInHouse], HouseBrankas[playerid][housebrankasTemp], requested);

            new invstr[1024];
            mysql_format(g_SQL, invstr, sizeof(invstr),
                "INSERT INTO `player_housestorage` (`ID`,`hsItemName`,`hsItemModel`,`hsItemQuantity`) VALUES(%d,'%e',%d,%d) ON DUPLICATE KEY UPDATE `hsItemQuantity` = `hsItemQuantity` + %d",
                AccountData[playerid][pID],
                HouseBrankas[playerid][housebrankasTemp],
                HouseBrankas[playerid][housebrankasModel],
                requested,
                requested);
        ...
        case DIALOG_HOUSEVAULT_WITHDRAW:
        {
            ...
            if(cache_num_rows())
            {
                ...
                ShowPlayerDialog(playerid, DIALOG_HOUSEVAULT_OUT, DIALOG_STYLE_INPUT, ""TTR"Astral City Roleplay "WHITE"- Brankas Bengkel",
                shstr, "Input", "Batal");
            }
        }
        case DIALOG_HOUSEVAULT_OUT:
        {
            ...
            if(isnull(inputtext))
            {
                ...
                ShowPlayerDialog(playerid, DIALOG_HOUSEVAULT_OUT, DIALOG_STYLE_INPUT, ""TTR"Astral City Roleplay "WHITE"- Brankas Rumah", 
                shstr, "Input", "Batal");
                return 1;
            }
            ...
            switch(listitem)
            {
                case 3:
                {
                    ...
                    if(count == 0)
                    {
                        ...
                        return ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""TTR"Astral City Roleplay "WHITE"- Undang Teman", 
                        "Tidak ada player yang berada dekat dengan pintu rumah anda!", "Tutup", "");
                    }

                    ShowPlayerDialog(playerid, DIALOG_HOUSE_INVITE, DIALOG_STYLE_LIST, ""TTR"Astral City Roleplay "WHITE"- Undang Teman", frmxt, "Undang", "Batal");
                }
                case 4:
                {
                ...
                {
                    if(!House_HaveAccess(playerid, id)) return ShowTDN(playerid, NOTIFICATION_ERROR, "Hanya pemilik rumah dan pemegang kunci yang dapat mengakses!");
                    ShowPlayerDialog(playerid, DIALOG_HOUSEVAULT, DIALOG_STYLE_LIST, ""TTR"Astral City Roleplay "WHITE"- Brankas Rumah",
                    "Simpan Barang\n"GRAY"Ambil Barang", "Pilih", "Batal");
                    AccountData[playerid][menuShowed] = true;
                }
                case 8:// Putar Musik
                {
                    Dialog_Show(playerid, DialogHouseMusik, DIALOG_STYLE_INPUT, ""TTR"Astral City Roleplay "WHITE"- House Musik",
        ...
        case DIALOG_HOUSE_INVITE:
        {
            ...
            ShowPlayerDialog(targetid, DIALOG_HOUSE_INVITECONF, DIALOG_STYLE_MSGBOX, ""TTR"Astral City Roleplay "WHITE"- Undang Teman", 
        ...
        case DIALOG_HKEYS:
        {
            ...
            if(HouseMemberName[playerid][listitem][0] == EOS) return ShowPlayerDialog(playerid, DIALOG_HKEYS_ADD, DIALOG_STYLE_INPUT, ""TTR"Astral City Roleplay "WHITE"- Bagikan Kunci",
            ...
            ShowPlayerDialog(playerid, DIALOG_HKEYS_REMOVE, DIALOG_STYLE_MSGBOX, ""TTR"Astral City Roleplay"WHITE"- Pemegang Kunci", shstr, "Iya", "Tidak");
        ...
        case DIALOG_HKEYS_ADD:
        {
            ...
            if(isnull(inputtext)) return ShowPlayerDialog(playerid, DIALOG_HKEYS_ADD, DIALOG_STYLE_INPUT, ""TTR"Astral City Roleplay "WHITE"- Bagikan Kunci",
            "Error: Tidak dapat diisi kosong!\nMohon masukkan playerid yang ingin diberikan kunci rumah:", "Input", "Kembali");

            if(!IsNumeric(inputtext)) return ShowPlayerDialog(playerid, DIALOG_HKEYS_ADD, DIALOG_STYLE_INPUT, ""TTR"Astral City Roleplay "WHITE"- Bagikan Kunci",
            "Error: Hanya dapat diisi angka!\nMohon masukkan playerid yang ingin diberikan kunci rumah:", "Input", "Kembali");
...
CMD:buyhouse(playerid, params[])
{
    ...
    {
        if(HouseData[i][hsOwnerID] == AccountData[playerid][pID])
            owned_houses++;
    }

    new max_slots = AccountData[playerid][pHouseSlotPlus];
    if(max_slots < 1) max_slots = 1;

    if(owned_houses >= max_slots)
        return ShowTDN(playerid, NOTIFICATION_ERROR, sprintf("Anda telah mencapai batas maksimal kepemilikan rumah! (Maks: %d)", max_slots));

    HouseData[id][hsOwnerID] = AccountData[playerid][pID];
    format(HouseData[id][hsOwner], MAX_PLAYER_NAME, "%s", AccountData[playerid][pName]);
