...
hook OnPlayerKeyStateChange(playerid, newkeys, oldkeys)
{
    if(newkeys & KEY_YES && GetPlayerState(playerid) == PLAYER_STATE_ONFOOT)
    {
        ...
        {
            Dialog_Show(playerid, RUSUN_BRANKAS, DIALOG_STYLE_LIST, ""TTR"Astral City Roleplay "WHITE"- %s",
            "Undang\
            \n"GRAY"Pakaian\
            \nMembuang Pakaian\
        ...
        if ((rsid = Rusun_Nearest(playerid)) != -1)
        {
            if (RusunsData[rsid][rOwnerID] != 0)
            {
                if (RusunsData[rsid][rOwnerID] != AccountData[playerid][pID])
                {
                    ...
                    ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, sprintf(""TTR"Astral City Roleplay "WHITE"- %s", RusunsData[rsid][rName]), 
                    "Sudah dimiliki warga lain!", "Tutup", "");
                }
                else
                {
                    AccountData[playerid][pInRusun] = rsid;
                    Dialog_Show(playerid, RUSUN_OWNED, DIALOG_STYLE_LIST, sprintf(""TTR"Astral City Roleplay "WHITE"- %s", RusunsData[rsid][rName]),
                    "Masuk\
            ...
            {
                AccountData[playerid][pInRusun] = rsid;
                Dialog_Show(playerid, RUSUN_MENU, DIALOG_STYLE_TABLIST, sprintf(""TTR"Astral City Roleplay "WHITE"- %s", RusunsData[rsid][rName]),
...
Dialog:RUSUN_BRANKAS(playerid, response, listitem, inputtext[])
{
    ...
    switch(listitem)
    {
        case 0:
        {
            ...
            Dialog_Show(playerid, RUSUN_INVITE, DIALOG_STYLE_LIST, ""TTR"Astral City Roleplay "WHITE"- Undang Teman", frmxt, "Pilih", "Batal");
        }
        case 1:
        {
        ...
        case 3:
        {
            Dialog_Show(playerid, RUSUN_BROPTION, DIALOG_STYLE_LIST, ""TTR"Astral City Roleplay "WHITE"- Brankas",
...
Dialog:RUSUN_INVITE(playerid, response, listitem, inputtext[])
{
    ...
    if ((id = RusunInside(playerid)) != -1)
    {
        ...
        Dialog_Show(targetid, RUSUN_INVITECONF, DIALOG_STYLE_MSGBOX, ""TTR"Astral City Roleplay "WHITE"- Undang Teman",
    ...
    switch(listitem)
    {
        case 0: //deposit
        {
            ...
            if(cache_num_rows() > 0)
            {
                ...
                Dialog_Show(playerid, RUSUNVAULT_DEPOSIT, DIALOG_STYLE_TABLIST_HEADERS, ""TTR"Astral City Roleplay "WHITE"- Brankas", str, "Pilih", "Batal");
            }
            else 
            {
                PlayerPlaySound(playerid, 5206, 0.0, 0.0, 0.0);
        ...
        case 1: //withdraw
        {
            ...
            if(cache_num_rows() > 0)
            {
                ...
                Dialog_Show(playerid, RUSUNVAULT_WITHDRAW, DIALOG_STYLE_TABLIST_HEADERS, ""TTR"Astral City Roleplay "WHITE"- Brankas", str, "Pilih", "Batal");
            }
            else 
            {
                return ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""TTR"Astral City Roleplay "WHITE"- Brankas",
...
Dialog:RUSUNVAULT_DEPOSIT(playerid, response, listitem, inputtext[])
{
    ...
    if(cache_num_rows() > 0)
    {
        if(listitem >= 0 && listitem < cache_num_rows())
        {
            ...
            Dialog_Show(playerid, RUSUNVAULT_IN, DIALOG_STYLE_INPUT, ""TTR"Astral City Roleplay "WHITE"- Brankas",
            shstr, "Input", "Batal");
        }
    }
    return 1;
...
Dialog:RUSUNVAULT_IN(playerid, response, listitem, inputtext[])
{
    ...
    RusunProcessing[playerid] = true; // lock paling awal
    ...
    {
        ShowDialogInvalid(playerid, "Mohon masukkan jumlah valid!");
        RusunProcessing[playerid] = false;
        return 1;
    }
    ...
    // update DB pakai ON DUPLICATE KEY UPDATE
    new invstr[1024];
    mysql_format(g_SQL, invstr, sizeof(invstr),
        "INSERT INTO `player_rusunstorage` (`ID`,`rsItemName`,`rsItemModel`,`rsItemQuantity`) "
        "VALUES(%d,'%e',%d,%d) "
        "ON DUPLICATE KEY UPDATE `rsItemQuantity` = `rsItemQuantity` + %d",
        AccountData[playerid][pID],
        RusunBrankas[playerid][rusunbrankasTemp],
        RusunBrankas[playerid][rusunbrankasModel],
        requested,
        requested);
...
Dialog:RUSUNVAULT_WITHDRAW(playerid, response, listitem, inputtext[])
{
    ...
    if(cache_num_rows() > 0)
    {
        if(listitem >= 0 && listitem < cache_num_rows())
        {
            ...
            Dialog_Show(playerid, RUSUNVAULT_OUT, DIALOG_STYLE_INPUT, ""TTR"Astral City Roleplay "WHITE"- Brankas",
            shstr, "Input", "Batal");
        }
    }
    return 1;
