...
hook OnPlayerKeyStateChange(playerid, newkeys, oldkeys)
{
    ...
    if(PRESSED(KEY_YES) && GetPlayerState(playerid) == PLAYER_STATE_ONFOOT)
    {
        ...
        if(hpid != -1)
        {
            ...
            ShowPlayerDialog(playerid, DIALOG_HOUSEHELIPAD_OUT, DIALOG_STYLE_TABLIST_HEADERS, sprintf(""TTR"Astral City Roleplay "WHITE"- Garasi Helipad %d", hpid), list, "Pilih", "Batal");
        }
    }
    if(newkeys & KEY_CROUCH && GetPlayerState(playerid) == PLAYER_STATE_DRIVER)
    {
