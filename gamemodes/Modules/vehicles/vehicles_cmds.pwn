...
CMD:sellveh(playerid, params[]) 
{
	...
	foreach(new i : PvtVehicles) if (PlayerVehicle[i][pVehExists])
	{
		if(PlayerVehicle[i][pVehPhysic] == vid)
		{
			...
			Dialog_Show(playerid, SellVehMenu, DIALOG_STYLE_LIST, ""TTR"Astral City Roleplay "WHITE"- Metode Pembayaran", "Cash\nSaldo Rekening", "Pilih", "Batal");
...
Dialog:SellVehMenu(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		...
		switch(listitem)
		{
			case 0: // Cash
			{
				Dialog_Show(otherid, BuyvehCashMenu, DIALOG_STYLE_MSGBOX, ""TTR"Astral City Roleplay "WHITE"- Beli Kendaraan",
			...
			case 1: // Bank
			{
				Dialog_Show(otherid, BuyvehRekeningMenu, DIALOG_STYLE_MSGBOX, ""TTR"Astral City Roleplay "WHITE"- Beli Kendaraan",
				""WHITE"%s menawarkan anda {FFFF00}%s{FFFFFF} seharga {00FF00}%s\
...
CMD:avehlist(playerid, params[])
{
	...
	if(found)
		ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""TTR"Astral City Roleplay "WHITE"- Player Vehicle", CMDString, "Tutup", "");
	else 
...
CMD:myv(playerid, params[])
{
	...
	if(!GetOwnedVeh(playerid)) return ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""TTR"Astral City Roleplay "WHITE"- Kepemilikan Kendaraan", "Anda tidak memiliki kendaraan apapun!", "Tutup", "");
	...
	ShowPlayerDialog(playerid, DialogMyVeh, DIALOG_STYLE_TABLIST_HEADERS, ""TTR"Astral City Roleplay "WHITE"- Kepemilikan Kendaraan", CMDString, "Cari", "Cancel");
	return 1;
}
...
CMD:windows(playerid, params[])
{
	...
	Dialog_Show(playerid, WindowsControl, DIALOG_STYLE_LIST, ""TTR"Astral City Roleplay "WHITE"- Vehicle Windows Control", "Driver Seat\nPassenger Seat\nBack Left\nBack Right\nClose All", "Choose", "Close");
	return 1;
}
