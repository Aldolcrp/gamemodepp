...
stock Inventory_Remove(playerid, item[], quantity = 1)
{
	...
	if (itemid != -1)
	{
	    if (InventoryData[playerid][itemid][invQuantity] > 0)
	    {
            ...
            else if(!strcmp(item, "Lockpick"))
            {
                AccountData[playerid][pBeratItem] -= quantity*0.001;
            }
            else if(!strcmp(item, "Laptop Hacking"))
            {
                AccountData[playerid][pBeratItem] -= quantity*0.001;
            }
            else if(!strcmp(item, "Bomb"))
            {
                AccountData[playerid][pBeratItem] -= quantity*0.001;
            }
...
public OpenInventory(playerid)
{
    ...
    if(count)
    {
        ...
        ShowPlayerDialog(playerid, DIALOG_INVENTORY, DIALOG_STYLE_TABLIST_HEADERS, ""TTR"Astral City Roleplay "WHITE"- Inventory", shstr, "Pilih", "Batal");
        return 1;
    }
...
public OnPlayerUseItem(playerid, itemid, itemname[])
{
    ...
    else if(!strcmp(itemname, "Race Tablet", true))
    {
    ...
    else if(!strcmp(itemname, "Masker", true))
    {
    ...
    else if(!strcmp(itemname, "Lockpick", true))
    {
        ...
        if(gettime() < LockpickCooldown[playerid])
        {
            ...
            ShowTDN(playerid, NOTIFICATION_ERROR, 
                sprintf("Anda harus menunggu %d detik sebelum mencoba lagi.", remaining));
            return 1;
        }

        foreach(new iterpv : PvtVehicles)
        {
            if(PlayerVehicle[iterpv][pVehExists] && PlayerVehicle[iterpv][pVehPhysic] == veh)
            {
                ...
                SendRPMeAboveHead(playerid, "Mencoba membobol kendaraan di depan menggunakan lockpick", X11_PLUM1);
                SetTimerEx("DelayLockpick", 1500, false, "d", playerid);
            }
        }
    }
    ...
    else if(!strcmp(itemname, "Tools Kit", true))
    {
        ...
        new vehid = Vehicle_Nearest(playerid);
        if(vehid == -1)
            return ShowTDN(playerid, NOTIFICATION_ERROR, "Tidak ada kendaraan apapun disekitar!");
        ...
        PlayerMinigameType[playerid] = 2;
        PlayerTargetVeh[playerid] = PlayerVehicle[vehid][pVehPhysic];
        ...
        ShowTDN(playerid, NOTIFICATION_INFO, "Minigame tools kit telah di mulai tekan tombol yang muncul");
        ...
        for(new i = 0; i < 3; i++) PlayerTextDrawShow(playerid, VehicleRepairTD[playerid][i]);
    }
    else if(!strcmp(itemname, "Repair Kit", true))
    {
        ...
        ShowTDN(playerid, NOTIFICATION_INFO, "Minigame repair kit telah di mulai tekan tombol yang muncul");
...
CMD:itemlist(playerid, params[])
{
    ...
    return ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_LIST, ""TTR"Astral City Roleplay "WHITE"- Nama Barang", string, "Select", "Cancel");
}
...
public OpeningBackpack(playerid)
{
    ...
    if(AccountData[playerid][ActivityTime] >= 115)
    {
        ...
        VehicleStaterpack_Create(playerid, 418, FACTION_NONE, x, y, z, random(255), random(255), 60000);
	 	ShowTDN(playerid, NOTIFICATION_SUKSES, "Berhasil mengambil benefit kendaraan Moonbeam!");
