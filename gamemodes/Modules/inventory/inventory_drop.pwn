...
Float: GetItemWeight(itemname[], quantity)
{
    ...
    else if(!strcmp(itemname, "Race Tablet"))
    {
        weight = quantity*0.20;
    }
 	...
 	if(rows)
  	{
    	forex(i, rows)
		{
			...
			DroppedItems[i][droppedObject] = CreateDynamicObject(2663, DroppedItems[i][droppedPos][0], DroppedItems[i][droppedPos][1], DroppedItems[i][droppedPos][2] + 0.2, 0.0, 0.0, 0.0, DroppedItems[i][droppedWorld], DroppedItems[i][droppedInt]);
	...
	switch(dialogid)
	{
		case DialogDropItem:
		{
			...
			if(response)
			{
				...
				ShowPlayerDialog(playerid, DialogDropItem, DIALOG_STYLE_INPUT, ""TTR"Astral City Roleplay "WHITE"- Drop Barang", str, "Drop", "Cancel");
...
/*hook OnPlayerKeyStateChange(playerid, newkeys, oldkeys)
{
    if(newkeys & KEY_WALK && GetPlayerSpecialAction(playerid) == SPECIAL_ACTION_DUCK)
    {
        ...
        if(id != -1)
        {
            ...
            else ShowPlayerDialog(playerid, DIALOG_ITEM_PICKUP, DIALOG_STYLE_TABLIST_HEADERS, ""TTR"Astral City Roleplay "WHITE"- Pickup Item", shstr, "Ambil", "Batal");
        }
    }
	return 1;
}*/
...
InventoryDrop(playerid) {
    ...
    for (new i = 0; i < MAX_DROPPED_ITEMS; i ++) 
    {
        if(DroppedItems[i][droppedModel] && 
           IsPlayerInRangeOfPoint(playerid, 3.0, DroppedItems[i][droppedPos][0], DroppedItems[i][droppedPos][1], DroppedItems[i][droppedPos][2]) && 
           GetPlayerInterior(playerid) == DroppedItems[i][droppedInt] && 
           GetPlayerVirtualWorld(playerid) == DroppedItems[i][droppedWorld]) 
        {
    ...
    for(new i; i < 8; i++) 
    {
        PlayerTextDrawHide(playerid, BoxInv2[playerid][i]);
        ...
        PlayerTextDrawHide(playerid, LineInv2[playerid][i]);
        ...
        if(id_tanah[i] != -1) 
        {
            ...
            PlayerTextDrawShow(playerid, LineInv2[playerid][i]);
        }
    }
