...
RefreshLabelPoms(playerid)
{
    for(new x = 0; x < sizeof(RefillArea); x++)
    {
        ...
        {
            UpdateDynamic3DTextLabelText(pomLabel[playerid][x], -1, "Tekan "GREEN"Y"WHITE" untuk mengisi BBM\n""Tekan "GREEN"H"WHITE" untuk membeli Jerigen\n""Harga:"DARKGREEN" $1500");
        }
        else
        {
...
hook OnPlayerKeyStateChange(playerid, newkeys, oldkeys)
{
    ...
    if(newkeys & KEY_CTRL_BACK && GetPlayerState(playerid) == PLAYER_STATE_ONFOOT)
    {
        if(IsRefillArea(playerid))
        {
            if(AccountData[playerid][pMoney] < 1500) 
