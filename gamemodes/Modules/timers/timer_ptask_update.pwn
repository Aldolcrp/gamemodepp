FUNC:: PlayerUpdate(playerid)
{
	if (AccountData[playerid][IsLoggedIn] && AccountData[playerid][pSpawned])
	{
		if (AccountData[playerid][pSpawned])
		{
			...
			{
				new scoremath = (AccountData[playerid][pLevel] * 2);
				...
				{
					if (AccountData[playerid][pHours] >= scoremath)
					{
						static string[144];
						...
						format(string, sizeof(string), "[Level Up] "RED"%s(%d) "WHITE"telah mencapai "YELLOW"level %d, "WHITE"selamat ya!", 
							AccountData[playerid][pName], playerid, AccountData[playerid][pLevel]);
		...
		if(AccountData[playerid][pSpec] != -1)
		{
			...
			PlayerTextDrawSetString(playerid, SpectatorInfoTD[playerid][0], sprintf("Nama IC/UCP: %s (%s)", AccountData[targetid][pName], AccountData[targetid][pUCP]));
			PlayerTextDrawSetString(playerid, SpectatorInfoTD[playerid][2], sprintf("ID: %d", targetid));
			PlayerTextDrawSetString(playerid, SpectatorInfoTD[playerid][3], sprintf("Cash Money: %s", FormatMoney(AccountData[targetid][pMoney])));
			PlayerTextDrawSetString(playerid, SpectatorInfoTD[playerid][4], sprintf("Health: %d", GetHealth(targetid)));
			PlayerTextDrawSetString(playerid, SpectatorInfoTD[playerid][5], sprintf("Armour: %d", GetArmor(targetid)));
			PlayerTextDrawSetString(playerid, SpectatorInfoTD[playerid][6], sprintf("Interior: %d", GetPlayerInterior(targetid)));
			PlayerTextDrawSetString(playerid, SpectatorInfoTD[playerid][7], sprintf("World: %d", GetPlayerVirtualWorld(targetid)));
			PlayerTextDrawSetString(playerid, SpectatorInfoTD[playerid][8], sprintf("Stress %d", AccountData[targetid][pStress]));
			PlayerTextDrawSetString(playerid, SpectatorInfoTD[playerid][9], sprintf("Fps: %d Ping: %dms", GetPlayerFPS(targetid), GetPlayerPing(targetid)));
			PlayerTextDrawSetString(playerid, SpectatorInfoTD[playerid][14], sprintf("Hungry: %d", AccountData[targetid][pHunger]));
			PlayerTextDrawSetString(playerid, SpectatorInfoTD[playerid][15], sprintf("Thirst: %d", AccountData[targetid][pThirst]));
		}
		//times
		static year, month, day, hour, minute, second;
		gettime(hour, minute, second);
		getdate(year, month, day);
		PlayerTextDrawSetString(playerid, InfoDate[playerid][5], sprintf("%02d:%02d", hour, minute, second));
		PlayerTextDrawSetString(playerid, InfoDate[playerid][0], sprintf("%02d %02d %d", day, month, year));
		PlayerTextDrawSetString(playerid, InfoDate[playerid][9], sprintf("%d", playerid));
...
/* Textdraw Death */
		if(AccountData[playerid][pInjured])
		{
			if(AccountData[playerid][pInjuredTime] > 0)
			{
				...
				new puluhan_menit = minutes / 10;
				new satuan_menit  = minutes % 10;
				new puluhan_detik = seconds / 10;
				new satuan_detik  = seconds % 10;

				new str_td[2];
				new countems;
				new jumlahems[50];

				format(str_td, sizeof(str_td), "%d", satuan_detik);
				PlayerTextDrawSetString(playerid, InjuredSytems[playerid][39], str_td);

				format(str_td, sizeof(str_td), "%d", puluhan_detik);
				PlayerTextDrawSetString(playerid, InjuredSytems[playerid][36], str_td);

				format(str_td, sizeof(str_td), "%d", satuan_menit);
				PlayerTextDrawSetString(playerid, InjuredSytems[playerid][33], str_td);

				format(str_td, sizeof(str_td), "%d", puluhan_menit);
				PlayerTextDrawSetString(playerid, InjuredSytems[playerid][30], str_td);

				foreach(new i : Player) if (IsPlayerConnected(i))
				{
					if(AccountData[i][pDutyEms]) countems++;
				}
				format(jumlahems, sizeof(jumlahems), "%d", countems);
				PlayerTextDrawSetString(playerid, InjuredSytems[playerid][25], jumlahems);
				for(new i = 0; i < 42; i ++)
				{
					PlayerTextDrawShow(playerid, InjuredSytems[playerid][i]);
				}
		...
		if(AccountData[playerid][pInjured] == 0 && AccountData[playerid][pGender] != 0) 
		{
			...
			if(++ AccountData[playerid][pHungerTime] >= 725)
			{
			...
			if(++ AccountData[playerid][pThirstTime] >= 700)
			{
		...
		if(!GetPVarInt(playerid, "HbeHiden"))
		{
			...
			Health = GetHealth(playerid) * 79.000/100.0;
			Armour = GetArmor(playerid) * 79.000/100.0;
			Hunger = AccountData[playerid][pHunger] * -18.000/100.0;
			Thirst = AccountData[playerid][pThirst] * -18.000/100.0;
			Stress = AccountData[playerid][pStress] * -18.000/100.0;

			format(string, sizeof(string), "%.0f%", AccountData[playerid][pHealth]);
			PlayerTextDrawSetString(playerid, HbeNewv2[playerid][5], string);

			format(string, sizeof(string), "%.0f%", AccountData[playerid][pArmour]);
			PlayerTextDrawSetString(playerid, HbeNewv2[playerid][4], string);
			
			PlayerTextDrawTextSize(playerid, HbeNewv2[playerid][2], Health, 4.0000);
			PlayerTextDrawShow(playerid, HbeNewv2[playerid][2]);

			PlayerTextDrawTextSize(playerid, HbeNewv2[playerid][3], Armour, 4.0000);
			PlayerTextDrawShow(playerid, HbeNewv2[playerid][3]);

			PlayerTextDrawTextSize(playerid, HbeNewv2[playerid][18], 3.000, Hunger);
			PlayerTextDrawShow(playerid, HbeNewv2[playerid][18]);

			PlayerTextDrawTextSize(playerid, HbeNewv2[playerid][19], 3.000, Thirst);
			PlayerTextDrawShow(playerid, HbeNewv2[playerid][19]);

			PlayerTextDrawTextSize(playerid, HbeNewv2[playerid][20], 3.000, Stress);
			PlayerTextDrawShow(playerid, HbeNewv2[playerid][20]);
		}
...
stock PlayerCompassUpdate(playerid, vehid)
{
    ...
    static frmtcompass[32];
    if (Ang >= 348.75 || Ang < 11.25) frmtcompass = "Utara";
    else if (Ang < 56.25) frmtcompass = "Barat Laut";
    else if (Ang < 101.25) frmtcompass = "Barat";
    else if (Ang < 146.25) frmtcompass = "Barat Daya";
    else if (Ang < 191.25) frmtcompass = "Selatan";
    else if (Ang < 236.25) frmtcompass = "Tenggara";
    else if (Ang < 281.25) frmtcompass = "Timur";
    else if (Ang < 326.25) frmtcompass = "Timur Laut";
    else frmtcompass = "Utara";

    PlayerTextDrawSetString(playerid, SPEEDONEW[playerid][4],
        sprintf("%s // %02d:%02d", GetLocation(X, Y, Z), hours, minutes));
    return 1;
}
...
FUNC::VehicleTDUpdate(playerid)
{
    ...
    if(!IsValidVehicle(vehid)) return 1;
    ...
    if(!GetPVarInt(playerid, "SpeedoHiden"))
    {
        if(IsPlayerInAnyVehicle(playerid) && GetPlayerState(playerid) == PLAYER_STATE_DRIVER)
        {
			...
			new Float: vehicleHealth, playerVehicleId = GetPlayerVehicleID(playerid);
			GetVehicleHealth(playerVehicleId, vehicleHealth);

			new Float:speed = GetVehicleSpeed(vehid);
			PlayerTextDrawSetString(playerid, SPEEDONEW[playerid][2], sprintf("%03d", floatround(speed)));
            PlayerTextDrawTextSize(playerid, SPEEDONEW[playerid][8], 8.500, VehicleCore[vehid][vCoreFuel] * (-41.500/100));
			PlayerTextDrawTextSize(playerid, SPEEDONEW[playerid][13], 8.500, vehicleHealth * (-41.600/1000));

            PlayerTextDrawShow(playerid, SPEEDONEW[playerid][8]);
            PlayerTextDrawShow(playerid, SPEEDONEW[playerid][2]);
			PlayerTextDrawShow(playerid, SPEEDONEW[playerid][13]);
...
FUNC:: AlertNeeds_Player(playerid)
{
	...
	if(AccountData[playerid][pHunger] <= 20) {
		SendClientMessageEx(playerid, -1, ""ASTRALCITYDOT"Karakter anda merasakan lapar // %d Persen", AccountData[playerid][pHunger]);
	}
	if(AccountData[playerid][pThirst] <= 20) {
		SendClientMessageEx(playerid, -1, ""ASTRALCITYDOT"Karakter anda merasakan haus // %d Persen", AccountData[playerid][pThirst]);
	}
	if(AccountData[playerid][pStress] >= 90) {
		SendClientMessageEx(playerid, -1, ""ASTRALCITYDOT"Karakter anda ingin mengalami stress // %d Persen", AccountData[playerid][pStress]);
	}
	return 1;
}