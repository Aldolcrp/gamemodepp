...
CMD:changebet(playerid, params[])
{
	if(Gambling[playerid] == G_STATE_READY)
	{
		Dialog_Show(playerid, ChangeBetSpin, DIALOG_STYLE_INPUT, ""TTR"Astral City Roleplay "WHITE"- Change Bet", 
...
Dialog:ChangeBetSpin(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		...
		if(bet < MIN_BET) return Dialog_Show(playerid, ChangeBetSpin, DIALOG_STYLE_INPUT, ""TTR"Astral City Roleplay "WHITE"- Change Bet", 
		""WHITE"ERROR: Minimal Bet $10!\n"WHITE"Bet anda saat ini: "GREEN"%s\n\n"WHITE"Mohon masukkan berapa jumlah yang ingin anda pasang dalam taruhan ini:\nMin Bet: "GREEN"$10\n"WHITE"Max Bet: "GREEN"$10",
		"Bet", "Cancel", FormatMoney(Bet[playerid]));

		if(bet > MAX_BET) return Dialog_Show(playerid, ChangeBetSpin, DIALOG_STYLE_INPUT, ""TTR"Astral City Roleplay "WHITE"- Change Bet", 
