...
stock Pemerintah_OpenStorage(playerid, facid)
{
    ...
    if(!count3) return Dialog_Show(playerid, ShowOnly, DIALOG_STYLE_MSGBOX, ""TTR"Astral City Roleplay "WHITE"- Brankas Pemerintah", "Tidak ada barang apapun dibrankas saat ini!", "Tutup", "");
    Dialog_Show(playerid, PemerintahBrankasItem, DIALOG_STYLE_TABLIST_HEADERS, ""TTR"Astral City Roleplay "WHITE"- Brankas Pemerintah", str, "Pilih", "Batal");
    return 1;
}
...
Dialog:PemerintahBrankasItem(playerid, response, listitem, inputtext[])
{
    if(response)
    {
        ...
        if(PemerintahStorage[facid][AccountData[playerid][pListItemGudang]][ItemExists])
        {
            ...
            Dialog_Show(playerid, PemerintahWithdraw, DIALOG_STYLE_INPUT, ""TTR"Astral City Roleplay "WHITE"- Brankas Pemerintah", str, "Submit", "Back");
        }
    }
    return 1;
}
