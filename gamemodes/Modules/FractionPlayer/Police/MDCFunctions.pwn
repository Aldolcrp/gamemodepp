...
NearMDTArea(playerid)
{
    ...
    if(IsPlayerInRangeOfPoint(playerid, 3.0, 1775.623, -1765.881, 14.105)) // Kantor Polisi
        return 1;
...
{
    Dialog_Show(playerid, MAIN_MDC, DIALOG_STYLE_LIST, ""TTR"Astral City Roleplay "WHITE"- MDT Dashboard",
    "1) Name Search\
    \n"GRAY"2) Plate Search\
...
hook OnGameModeInitEx()
{
    ...
    CreateDynamicPickup(1239, 23, 1775.623, -1765.881, 14.105, -1, -1, -1, 10.0);
	CreateDynamic3DTextLabel(""LIGHTGREY"[Mobile Data Terminal]\n"WHITE"Gunakan "YELLOW"'/mdt'"WHITE" untuk akses MDT", -1, 1775.623, -1765.881, 14.105 + 0.6, 10.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1);    
...
Dialog:MAIN_MDC(playerid, response, listitem, inputtext[])
{
    ...
    switch(listitem)
    {
        case 0: // name search
        {
            Dialog_Show(playerid, MDT_NAMESEARCH, DIALOG_STYLE_INPUT, ""TTR"Astral City Roleplay "WHITE"- MDT Name Search",
            "Kepolisian Kan\
        ...
        case 1: //plate search
        {
            Dialog_Show(playerid, MDT_PLATESEARCH, DIALOG_STYLE_INPUT, ""TTR"Astral City Roleplay "WHITE"- MDT Plate Search",
            "Kepolisian Kan\
            \nMobile Data Terminal (MDT)\
            \n============================\
            \nSelamat datang, %s %s!\n\
        ...
        case 5: {
            ...
            Dialog_Show(playerid, EmergencyList, DIALOG_STYLE_TABLIST_HEADERS, ""TTR"Astral City Roleplay "WHITE"- Emergency Calls", list, "Select", "Close");
        }
        case 6: {
...
{
    Dialog_Show(playerid, EmergencyOption, DIALOG_STYLE_LIST, ""TTR"Astral City Roleplay "WHITE"- Emergency Option",
    "1) Lacak Nomor\
    \n"GRAY"2) Kirim Pesan\
    \n3) Detail Laporan\
...
Dialog:EmergencyOption(playerid, response, listitem, inputtext[]) {
    if(response) {
        ...
        if(listitem == 1) {
            Dialog_Show(playerid, EmergencyPesan, DIALOG_STYLE_INPUT, ""TTR"Astral City Roleplay "WHITE"- Emergency Message", "Masukkan pesan yang akan anda kirim kepada pelapor.", "Kirim", "Tutup");
        }
        if(listitem == 2) {
            new string[712];
            ...
            Dialog_Show(playerid, EmergencyBack, DIALOG_STYLE_MSGBOX, ""TTR"Astral City Roleplay "WHITE"- Detail Emergency", string, "Close", "");
        }
        if(listitem == 3) {
