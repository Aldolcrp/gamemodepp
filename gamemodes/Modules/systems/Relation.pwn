    ...
    if(response)
    {
        switch(listitem)
        {
            case 0:
            {
                ...
                Dialog_Show(playerid, ShowOnly, DIALOG_STYLE_MSGBOX, ""TTR"Astral City Roleplay"WHITE" - Status Hubungan", output, "Close", "");
            }
            case 1:
            {
            ...
            case 2:
            {
                Dialog_Show(playerid, RelationInvites, DIALOG_STYLE_INPUT, ""TTR"Astral City Roleplay"WHITE" - Relation Invite", ""RED"[PERINGATAN] "WHITE"Kamu tidak bisa menjalin hubungan dengan sesama jenis!!!\n\n"YELLOW"(Masukan id/kantong player yang ingin kamu ajak menjalin hubungan)", "Select", "Close");
            }
            case 3:
            {   
                new targetid = IsPlayerNearPartner(playerid);
...
Dialog:RelationInvites(playerid, response, listitem, inputtext[])
{
    ...
    Dialog_Show(playerid, RelationMenu, DIALOG_STYLE_LIST, ""TTR"Astral City Roleplay"WHITE" - Relationship Invitation", "Invite to date\nMarry her!", "Select", "Cancel");
    return 1;
}
    ...
    if(response)
    {
        ...
        switch(listitem)
        {
            case 0:
            {
                ...
                Dialog_Show(userid, RelationInvite, DIALOG_STYLE_MSGBOX, ""TTR"Astral City Roleplay"WHITE" - Relationship Invitation", ""RED"%s "WHITE"has invited you to date them", "Accept", "Decline", name);
            }
            case 1:
            {
                if(IsPlayerInRangeOfPoint(playerid, 10.0, 1220.0037, -1446.7375, 45.2224))
                {
                    if(AccountData[playerid][pLove] == 100 && AccountData[userid][pLove] == 100)
                    {
                        if(AccountData[playerid][pPartnerID] == AccountData[userid][pID])
                        {
                            ...
                            Dialog_Show(userid, RelationInviteMarry, DIALOG_STYLE_MSGBOX, ""TTR"Astral City Roleplay"WHITE" - Relationship Invitation", ""RED"%s "WHITE"has proposed you to marry them", "Accept", "Decline", name);               
                        }
