stock bool:Anticheat_ShouldIgnore(playerid)
{
    if (!IsPlayerConnected(playerid)) return true;
    if (GetPlayerPing(playerid) > 400) return true;
    if (IsPlayerAdmin(playerid)) return true;
    if (AccountData[playerid][pAdminDuty] == 1) return true;
...
#define MAXIMUM_REPORTED_CHEAT          (10)
...
{
    if (Anticheat_ShouldIgnore(playerid)) return 0;
    if (code == 6)
    {
...
{
    if (Anticheat_ShouldIgnore(playerid)) return bool:false;
    
    if (code == 6)
    {
        new now = GetTickCount();
        return bool:(tele_pickup_ac_report_time[playerid] <= now);
    }
    return bool:true;
}
...
{
    if (Anticheat_ShouldIgnore(playerid)) return 1;
    Anticheat_SetReportTime(playerid, code, GetTickCount() + 5000);
    if (Anticheat_ShouldIgnore(playerid)) return 0;
    if (code == 6)
    {
