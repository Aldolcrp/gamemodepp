public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
	...
	if(dialogid == DIALOG_REGISTER)
    {
		...
		if (strlen(inputtext) <= 5) return ShowPlayerDialog(playerid, DIALOG_REGISTER, DIALOG_STYLE_PASSWORD, "Register - "TTR"Astral City Roleplay", ""WHITE"Buatlah kata sandi anda dengan benar\n"WHITE"NOTE: Kata sandi minimal 5 Karakter!\n\n"YELLOW"(Mohon isi dan selalu ingat Password anda di bawah ini):", "Register", "Tolak");
		
		if(!IsValidPassword(inputtext))
		{
			ShowTDN(playerid, NOTIFICATION_ERROR, "Sandi valid : A-Z, a-z, 0-9, _, [ ], ( )");
			ShowPlayerDialog(playerid, DIALOG_REGISTER, DIALOG_STYLE_PASSWORD, "Register - "TTR"Astral City Roleplay", ""WHITE"Kata sandi yang anda gunakan mengandung karakter yang tidak valid!\n\n"YELLOW"(Mohon isi dan selalu ingat Password anda di bawah ini):", "Register", "Tolak");
			return 1;
		}
	...
	if(dialogid == DIALOG_LOGIN)
	{
		...
		{
			SendAdminMessage(X11_RED, "[AntiCheat]: "YELLOW"(%s) "LIGHTGREY"Username mencoba merespon dialog kembali", GetName(playerid));
		...
		new input_pw[64];
		strcpy(input_pw, inputtext, sizeof(input_pw));
		new masked_pw[64];
		for(new i = 0; i < strlen(input_pw); i++)
		{
			masked_pw[i] = '-';
		}
		masked_pw[strlen(input_pw)] = '\0';

		new hashed_pass[65];
		SHA256_PassHash(input_pw, AccountData[playerid][pSalt], hashed_pass, sizeof(hashed_pass));

		SetPVarInt(playerid, "UCPBlacklist", 1);

		if(strcmp(hashed_pass, AccountData[playerid][pPassword]) == 0)
		{
			SetPVarInt(playerid, "PasswordVerified", 1);

			PlayerTextDrawSetString(playerid, TextdrawLoginUI[playerid][24], masked_pw);
			PlayerTextDrawShow(playerid, TextdrawLoginUI[playerid][24]);
			return 1;
		}
		else
		{
	...
	if(dialogid == DialogTrackMyVeh)
	{
		...
		if(isnull(inputtext))
		{
			ShowPlayerDialog(playerid, DialogTrackMyVeh, DIALOG_STYLE_INPUT, ""TTR"Astral City Roleplay "WHITE"- Lacak Kendaraan", 
	...
	if(dialogid == DialogToyEdit)
	{
		...
		if(response)
		{
			switch(listitem)
			{
				...
				case 2: // change bone
				{
					ShowPlayerDialog(playerid, DIALOG_TOYPOSISI, DIALOG_STYLE_LIST, ""TTR"Astral City Roleplay"WHITE"- Ubah Tulang(Bone)", 
					"Spine\
					\n"GRAY"Head\
					\nLeft Upper Arm\
				...
				case 3:
				{
					...
					ShowPlayerDialog(playerid, DIALOG_TOYPOSX, DIALOG_STYLE_INPUT, ""TTR"Astral City Roleplay "WHITE"- Edit Sumbu X", mstr, "Edit", "Cancel");
				}
				case 4:
				{
					new mstr[128];
					...
					ShowPlayerDialog(playerid, DIALOG_TOYPOSY, DIALOG_STYLE_INPUT, ""TTR"Astral City Roleplay "WHITE"- Edit Sumbu X", mstr, "Edit", "Cancel");
				}
				case 5:
				{
					new mstr[128];
					...
					ShowPlayerDialog(playerid, DIALOG_TOYPOSZ, DIALOG_STYLE_INPUT, ""TTR"Astral City Roleplay "WHITE"- Edit Sumbu Z", mstr, "Edit", "Cancel");
				}
				case 6:
				{
					new mstr[128];
					...
					ShowPlayerDialog(playerid, DIALOG_TOYPOSRX, DIALOG_STYLE_INPUT, ""TTR"Astral City Roleplay "WHITE"- Edit Rotasi X", mstr, "Edit", "Cancel");
				}
				case 7:
				{
					new mstr[128];
					...
					ShowPlayerDialog(playerid, DIALOG_TOYPOSRY, DIALOG_STYLE_INPUT, ""TTR"Astral City Roleplay "WHITE"- Edit Rotasi Y", mstr, "Edit", "Cancel");
				}
				case 8:
				{
					new mstr[128];
					...
					ShowPlayerDialog(playerid, DIALOG_TOYPOSRZ, DIALOG_STYLE_INPUT, ""TTR"Astral City Roleplay "WHITE"- Edit Rotasi Z", mstr, "Edit", "Cancel");
				}
				case 9:
				{
					new mstr[128];
					...
					ShowPlayerDialog(playerid, DIALOG_TOYPOSSX, DIALOG_STYLE_INPUT, ""TTR"Astral City Roleplay "WHITE"- Edit Skala X", mstr, "Edit", "Cancel");
				}
				case 10:
				{
					new mstr[128];
					...
					ShowPlayerDialog(playerid, DIALOG_TOYPOSSY, DIALOG_STYLE_INPUT, ""TTR"Astral City Roleplay "WHITE"- Edit Skala Y", mstr, "Edit", "Cancel");
				}
				case 11:
				{
					new mstr[128];
					...
					ShowPlayerDialog(playerid, DIALOG_TOYPOSSZ, DIALOG_STYLE_INPUT, ""TTR"Astral City Roleplay "WHITE"- Edit Skala Z", mstr, "Edit", "Cancel");
				}
				case 12: // remove toy
				{
				...
				{
					SendNearbyMessage(playerid, 10.0, -1, ""ASTRALCITY_ARWIN"[Cordinate By %s] "WHITE_E"PosX: %.3f | PosY: %.3f | PosZ: %.3f | PosRX: %.3f | PosRY: %.3f | PosRZ: %.3f",
		...
		else
		{
			...
			if(pToys[playerid][1][toy_model] == 0)
			{
				strcat(string, ""ASTRALCITY_ARWIN"Kacamata\n");
			}
			else strcat(string, ""ASTRALCITY_ARWIN"Kacamata\n");
			...
			if(pToys[playerid][3][toy_model] == 0)
			{
				strcat(string, ""ASTRALCITY_ARWIN"Tas/Koper\n");
			}
			else strcat(string, ""ASTRALCITY_ARWIN"Tas/Koper\n");

			ShowPlayerDialog(playerid, DIALOG_TOY, DIALOG_STYLE_LIST, ""TTR"Astral City Roleplay "WHITE"- Edit Fashion", string, "Select", "Cancel");
		}
		return 1;
	}
	...
	if(dialogid == DIALOG_TOYPOSX)
	{
		if(response)
		{
			...
			format(toys, sizeof(toys), ""TTR"Astral City Roleplay"WHITE" - Edit Fashion (%s)", minsty);
			format(string, sizeof string, "Pengaturan\tParameter\n");
			format(string, sizeof string, "%s"WHITE"Edit Posisi\t(Khusus PC)\n", string);
			format(string, sizeof string, "%s"ASTRALCITY_ARWIN"Edit Posisi\t(Khusus Android)\n", string);
			format(string, sizeof string, "%s"WHITE"Ubah Posisi Tulang\t(Bone)\n", string);
			...
			format(string, sizeof string, "%s"ASTRALCITY_ARWIN"Skala Z:\t%f\n", string, pToys[playerid][AccountData[playerid][toySelected]][toy_sz]);
			format(string, sizeof string, "%s"WHITE"Hapus\t(Pilih jika anda ingin menghapus permanent)\n", string);
			format(string, sizeof string, "%s"ASTRALCITY_ARWIN"Sembunyikan\t(Jika ingin disembunyikan)\n", string);
	...
	if(dialogid == DIALOG_DYNAMIC_HELP)
	{
		...
		switch(listitem)
		{
			case 0:// Actor
			{
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""TTR"Astral City Roleplay "WHITE"- Dynamic Actor",
				"Perintah\tKeterangan\n\
			...
			{
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""TTR"Astral City Roleplay "WHITE"- Dynamic Rents",
				"Perintah\tKeterangan\n\
			...
			case 2: // Dynamic Door
			{
				...
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""TTR"Astral City Roleplay "WHITE"- Dynamic Door", str, "Tutup", "");
			}
			case 3: // Fivem Label
			{
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""TTR"Astral City Roleplay "WHITE"- Dynamic Label",
				"Perintah\tKeterangan\n\
			...
			{
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""TTR"Astral City Roleplay "WHITE"- Dynamic Public Garage",
				"Perintah\tKeterangan\n\
			...
			case 5: //Tong Sampah
			{
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""TTR"Astral City Roleplay "WHITE"- Dynamic Trash",
				"Perintah\tKeterangan\n\
			...
			case 6: //Dynamic ATM
			{
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""TTR"Astral City Roleplay "WHITE"- Dynamic ATM", 
				"Perintah\tKeterangan\n\
			...
			case 7: //Kanabis
			{
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""TTR"Astral City Roleplay "WHITE"- Dynamic Ladang", 
				"Perintah\tKeterangan\n\
			...
			{
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""TTR"Astral City Roleplay "WHITE"- Dynamic Robbery", 
				"Perintah\tKeterangan\n\
			...
			case 10: //Hunting
			{
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""TTR"Astral City Roleplay "WHITE"- Dynamic Hunting", 
				"Perintah\tKeterangan\n\
			...
			case 11: //Gudang
			{
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""TTR"Astral City Roleplay "WHITE"- Dynamic Gudang", 
				"Perintah\tKeterangan\n\
			...
			{
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""TTR"Astral City Roleplay "WHITE"- Dynamic Houses", 
				"Perintah\tKeterangan\n\
			...
			{
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""TTR"Astral City Roleplay "WHITE"- Dynamic Icon",
				"Perintah\tKeterangan\n\
			...
			{
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""TTR"Astral City Roleplay "WHITE"- Dynamic Buttons",
				"Perintah\tKeterangan\n\
			...
			case 16: // Object
			{
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""TTR"Astral City Roleplay "WHITE"- Dynamic Object",
				"Perintah\tKeterangan\n\
	...
	if(dialogid == DIALOG_ADMIN_HELP)
	{
		...
		switch(listitem)
		{
			case 0: // Trial & The Stars
			{
				...
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""TTR"Astral City Roleplay "WHITE"- Trial Admin", 
				"Perintah\tKeterangan\n\
				/a\tAkses Chat Staff\n\
			...
			case 1: //AL2
			{
				...
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""TTR"Astral City Roleplay "WHITE"- Helper", 
				"Perintah\tKeterangan\n\
			...
			case 2:// AL3
			{
				...
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""TTR"Astral City Roleplay "WHITE"- Admin I", 
				"Perintah\tKeterangan\n\
			...
			case 3: //AL4
			{
				...
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""TTR"Astral City Roleplay "WHITE"- Admin II", 
				"Perintah\tKeterangan\n\
			...
			case 4: // AL5
			{
				...
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""TTR"Astral City Roleplay "WHITE"- Admin III", 
				"Perintah\tKeterangan\n\
			...
			case 5: //
			{
				...
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""TTR"Astral City Roleplay "WHITE"- Pengurus", 
				"Perintah\tKeterangan\n\
			...
			case 6: //management
			{
				...
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""TTR"Astral City Roleplay "WHITE"- Management", 
				"Perintah\tKeterangan\n\
	...
	if(dialogid == DIALOG_SETAMOUNT)
	{
		...
		PlayerTextDrawSetString(playerid, InventoryTD[playerid][0], inputtext);
	...
	if(dialogid == DIALOG_GOPAYWITHDRAW)
	{
		...
		if(amount > AccountData[playerid][pSaldoGopay]) return ShowPlayerDialog(playerid, DIALOG_GOPAYWITHDRAW, DIALOG_STYLE_INPUT, ""TTR"Astral City Roleplay"WHITE" - Tarik Saldo Gopay", "ERROR: Anda tidak memiliki saldo sebanyak itu\n\nInput jumlah saldo gopay yang ingin kamu tarik:", "Submit", "Cancel");
		if(amount < 1) return ShowPlayerDialog(playerid, DIALOG_GOPAYWITHDRAW, DIALOG_STYLE_INPUT, ""TTR"Astral City Roleplay"WHITE" - Tarik Saldo Gopay", "ERROR: Anda memasukan angka yang tidak Valid!\n\nInput jumlah saldo gopay yang ingin kamu tarik:", "Submit", "Cancel");
	...
	if(dialogid == DIALOG_MENU_PAYMENT)
	{
		if(!response) return 1;
		...
		switch(listitem)
		{
			case 0:
			{
				...
				new dialogStr[512], count = 0;
				new g_targetList[MAX_PLAYERS];
				giveMoneyTarget[playerid] = INVALID_PLAYER_ID;

				for(new i = 0; i < MAX_PLAYERS; i++)
				{
					if(i != playerid && IsPlayerConnected(i) && IsPlayerNearPlayer(playerid, i, 2.5))
					{
						format(dialogStr, sizeof(dialogStr), "%s%s - ID: %d\n", dialogStr, ReturnName(i), i);
						g_targetList[count++] = i;
					}
				}

				if(count == 0) return ShowTDN(playerid, NOTIFICATION_ERROR, "Tidak ada orang di sekitar anda.");

				for (new j = 0; j < count; j++)
					giveMoneyListTarget[playerid][j] = g_targetList[j];

				ShowPlayerDialog(playerid, DIALOG_GIVE_MONEY_PLAYER, DIALOG_STYLE_LIST, ""TTR"Astral City Roleplay -"WHITE" Give Money", dialogStr, "Pilih", "Tutup");
			}
			case 1:
			{
	...
	if(dialogid == DIALOG_GIVE_MONEY_PLAYER)
	{
		if(!response) return 1;

		new targetid = giveMoneyListTarget[playerid][listitem];

		if(!IsPlayerConnected(targetid) || !IsPlayerNearPlayer(playerid, targetid, 2.5))
			return ShowTDN(playerid, NOTIFICATION_ERROR, "Pemain tidak tersedia atau terlalu jauh.");

		giveMoneyTarget[playerid] = targetid;

		ShowPlayerDialog(playerid, DIALOG_GIVE_MONEY_AMOUNT, DIALOG_STYLE_INPUT, ""TTR"Astral City Give Money", "Masukkan jumlah uang yang ingin anda berikan:", "Kirim", "Batal");
		return 1;
	}
    if(dialogid == DIALOG_GIVE_MONEY_AMOUNT)
    {
        if(!response) return 1;

		static LastPayTime[MAX_PLAYERS];
        new otherid = giveMoneyTarget[playerid];
        new money = strval(inputtext);

        if(!IsPlayerConnected(otherid)) return ShowTDN(playerid, NOTIFICATION_ERROR, "Pemain tersebut tidak terkoneksi ke server!");
		if(gettime() - LastPayTime[playerid] < 3) return ShowTDN(playerid, NOTIFICATION_ERROR, "Tunggu beberapa detik sebelum memberikan uang lagi!");
		...
		ApplyAnimation(otherid, "DEALER", "DEALER_DEAL", 4.0, 0, 0, 0, 0, 0, 1);
		...
		ShowItemBox(playerid, sprintf("Removed %s", FormatMoney(money)), "Uang", 1212);
		ShowItemBox(otherid, sprintf("Received %s", FormatMoney(money)), "Uang", 1212);
		...
		if(response)
		{
			switch(listitem)
			{
				case 0:
				{
					...
					strcat(str, "duduk\nduduk2\nduduk3\nduduk4\nduduk5\nduduk6\njarteng\n");
					strcat(str, "pusing\nped\nped2\nped3\nped4\nped5\nped6\nped7\nped8\nped9\nped10\nped11\nped12\nped13\nped14\nped15\nped15\nkencing\nx\n");
					ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_LIST, ""TTR"Astral City Roleplay"WHITE" - List Animasi", str, "Close", "");
				}
				case 1:
				{
					ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_LIST, ""TTR"Astral City Roleplay "WHITE"- Emote Property",
					"TV\
					\nBan\
					\nBesbol\
					\nBasket\
				...
				case 2:
				{
					...
					ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_LIST, ""TTR"Astral City Roleplay "WHITE"- List Animasi Interaksi", str, "Close", "");
				}
			}
		}
	}
	if(dialogid == DIALOG_VERIFYCODE)
	{
		...
		if(isnull(inputtext))
		{
			format(shstr, sizeof(shstr), ""WHITE"Error: Tidak dapat diisi kosong!\n"WHITE"Selamat datang di "TTR"Astral City Roleplay\n"WHITE"UCP Ini telah terdaftar!\nNama UCP: "LIGHTGREEN"%s\
			\n"WHITE"Version: "TTR"ACRP v2.1.1\n"YELLOW"(Silahkan masukkan PIN yang dikirimkan oleh Bot Astral City ke Discord anda dibawah ini):", AccountData[playerid][pUCP]);
			ShowPlayerDialog(playerid, DIALOG_VERIFYCODE, DIALOG_STYLE_INPUT, ""TTR"Astral City Roleplay "WHITE"- Verifikasi Akun",
			shstr, "Input", "Batal");
			return 1;
		}
		...
		{
			format(shstr, sizeof(shstr), ""WHITE"Selamat datang di "TTR"Astral City Roleplay"WHITE"\nUCP ini telah terdaftar!\nNama UCP: "LIGHTGREEN"%s\n"WHITE"Version: "TTR"ACRP v2.1.1\n"YELLOW"(Silahkan untuk membuat password baru dibawah ini):", AccountData[playerid][pUCP]);
			ShowPlayerDialog(playerid, DIALOG_REGISTER, DIALOG_STYLE_PASSWORD, ""TTR"Astral City Roleplay "WHITE"- Pembuatan Password",
			shstr, "Input", "Batal");
			printf("[debug] OnPlayerVerifyCode(PID: %d UCP: %s) terpanggil", playerid, ReturnName(playerid));
			return 1;
		}

		format(shstr, sizeof(shstr), ""WHITE"Selamat datang di "TTR"Astral City Roleplay\n"WHITE"UCP Ini telah terdaftar!\nNama UCP: "LIGHTGREEN"%s\
		\n"WHITE"Version: "TTR"ACRP v2.0.2\n"YELLOW"(Silahkan masukkan PIN yang dikirimkan oleh Bot Astral City ke Discord anda dibawah ini):", AccountData[playerid][pUCP]);
		return ShowPlayerDialog(playerid, DIALOG_VERIFYCODE, DIALOG_STYLE_INPUT, ""TTR"Astral City Roleplay "WHITE"- Verifikasi Akun",
		shstr, "Input", "Batal");
	}
	...
	if(dialogid == DialogMyVeh)
	{
		...
		ShowPlayerDialog(playerid, DIALOG_MYV_MENU, DIALOG_STYLE_LIST, ""TTR"Astral City Roleplay "WHITE"- Kendaraan Saya", 
		"Cari Kendaraan\
	...
	if(dialogid == DIALOG_MYV_MENU)
	{
		...
		switch(listitem)
		{
			...
			case 1: // Detail kendaraan
			{
				...
				ShowPlayerDialog(playerid, DIALOG_VEHICLE_DETAIL, DIALOG_STYLE_INPUT, sprintf(""TTR"Astral City Roleplay "WHITE"- %s Vehicle Details", AccountData[playerid][pName]), strings, "Submit", "Batal");
			}
			case 2: { // Vallet
				new vehid = GetPVarInt(playerid, "ClickVehicle");
				new count = 0;
				...
				if(count >= 2) 
					return ShowTDN(playerid, NOTIFICATION_ERROR, "Anda tidak dapat menggunakan Vallet jika Trans Duty lebih dari 2 orang!");
	...
	if(dialogid == DIALOG_VEHICLE_DETAIL)
	{
		...
		(PlayerVehicle[vehid][pVehEngineUpgrade] != 1) ? ""RED"None" : ""GREEN"Upgrade", (PlayerVehicle[vehid][pVehBodyUpgrade] != 3) ? ""RED"None" : ""GREEN"Upgrade",
		GetFuel(PlayerVehicle[vehid][pVehPhysic]), vHealth, (PlayerVehicle[vehid][pVehEngineUpgrade] != 1) ? "1000.0" : "2000.0", PlayerVehicle[vehid][pVehBodyRepair]);
		ShowPlayerDialog(otherid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, sprintf(""TTR"Astral City Roleplay "WHITE"- %s Vehicle Details", AccountData[playerid][pName]), strings, "Tutup", "");
	}
	if(dialogid == DIALOG_CLICKPLAYER)
	{
		...
		switch(listitem)
		{
			case 0:
			{
				...
				ShowPlayerDialog(playerid, DIALOG_ADMIN_HELP, DIALOG_STYLE_LIST, ""TTR"Astral City Roleplay "WHITE"- Admin Help",
	...
	if(dialogid == DIALOG_BANNEDREASON)
	{
		...
		// SendEmbedMessage("Astral City Roleplay", shstr, "Kemanan Main #1", "", "", "1152229209498980392", 0xff8fd5);
	...
	if(dialogid == DIALOG_VIP_NAME)
	{
		...
		if(isnull(inputtext)) return ShowPlayerDialog(playerid, DIALOG_VIP_NAME, DIALOG_STYLE_INPUT, ""TTR"Astral City Roleplay "WHITE"- Vip Name Custom",
