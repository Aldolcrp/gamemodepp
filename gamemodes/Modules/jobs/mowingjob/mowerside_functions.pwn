...
hook OnScriptInit()
{
	...
	SetDynamicObjectMaterialText(tmpobjid, 0, "{ffffff}ASTRALCITY Mower", 130, "Arial", 60, 0, 0x00000000, 0x00000001, 1);
}
...
hook OnPlayerStateChange(playerid, newstate, oldstate)
{
    ...
    if(newstate == PLAYER_STATE_DRIVER && !IsMowerWorking(playerid) && IsMowerVehicles(vehicleid))
    {
        if(!AccountData[playerid][pMowerTime])
        {
            ...
            ShowPlayerDialog(playerid, DIALOG_MOWER_START, DIALOG_STYLE_MSGBOX, ""TTR"Astral City Roleplay "WHITE"- Mower Sidejob",
