    ...
    if(newkeys & KEY_YES && GetPlayerState(playerid) == PLAYER_STATE_ONFOOT)
    {
        if(AccountData[playerid][pJob] == JOB_KARGO)
        {
            if(IsPlayerInDynamicArea(playerid, PlayerKargoVars[playerid][KargoStartArea]))
            {
                ...
                Dialog_Show(playerid, DialogKargoRute, DIALOG_STYLE_LIST, ""TTR"Astral City Roleplay "WHITE"- Kargo", ""WHITE"Pertamina\n"ASTRALCITY_ARWIN"Barang", "Pilih", "Batal");
            }
        }
    }
    return 1;
    ...
    if(PlayerKargoVars[playerid][KargoStarted])
    {
        if(JobVehicle[AccountData[playerid][pJobVehicle]][Vehicle] != INVALID_VEHICLE_ID)
        {
            if(trailerid == PlayerKargoVars[playerid][KargoTrailer])
            {
                ...
                SendClientMessageEx(playerid, -1, ""ASTRALCITYDOT"Trailer terpasang kembali, silahkan melanjutkan perjalanan.");
            }
        }
    }
}
    ...
    if(PlayerKargoVars[playerid][KargoStarted])
    {
        if(JobVehicle[AccountData[playerid][pJobVehicle]][Vehicle] != INVALID_VEHICLE_ID)
        {
            if(trailerid == PlayerKargoVars[playerid][KargoTrailer])
            {
                ...
                SendClientMessageEx(playerid, -1, ""ASTRALCITYDOT"Trailer terlepas! anda memiliki waktu "RED"30 detik"WHITE" untuk menghubungkan kembali!");
            }
        }
    }
    return 1;
