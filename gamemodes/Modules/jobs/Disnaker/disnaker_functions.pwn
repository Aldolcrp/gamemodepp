...
hook OnGameModeInit()
{
    CreateDynamic3DTextLabel(""GREEN"[Y]"WHITE" Akses Disnaker", -1, 1480.525, -1802.977, 13.662, 15.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1);
...
Dialog:DISNAKER_MENU(playerid, response, listitem, inputtext[])
{
    if(response)
    {
        if(!IsPlayerInRangeOfPoint(playerid, 2.0, 1480.525, -1802.977, 13.662)) return ShowTDN(playerid, NOTIFICATION_ERROR, "Anda tidak berada di area disnaker!");
        switch(listitem)
        {
            case 0: // Ambil Job
            {
                ...
                countkargo, 
                countrecycler);
                ShowPlayerDialog(playerid, DIALOG_DISNAKER, DIALOG_STYLE_TABLIST_HEADERS, ""TTR"Astral City Roleplay "WHITE"- Disnaker", string, "Pilih", "Batal");
            }
            case 1: // Buat KTP
            {
                if(AccountData[playerid][Ktp]) return ShowTDN(playerid, NOTIFICATION_ERROR, "Anda sudah memiliki KTP!");
                if(AccountData[playerid][pMoney] < 5000) return ShowTDN(playerid, NOTIFICATION_ERROR, "Anda membutuhkan $5000 untuk membuat KTP!");
            ...
            case 4:
            {
                ShowPlayerDialog(playerid, DIALOG_SHOWROOM_SELL, DIALOG_STYLE_INPUT, ""TTR"Astral City Roleplay "WHITE"- Jual Kendaraan",
...
hook OnPlayerKeyStateChange(playerid, newkeys, oldkeys)
{
    ...
    {
        if(IsPlayerInRangeOfPoint(playerid, 2.0, 1480.525, -1802.977, 13.662))
        {
            ...
            if (AccountData[playerid][pPaycheck] > 0)
            {
                ...
                format(disnaker_str, sizeof(disnaker_str), "%sJual Kendaraan\tUntuk Menjual Kendaraan Anda ke pemerintah\n", disnaker_str);
            }
            else
            {
                format(disnaker_str, sizeof(disnaker_str), "%sAmbil Pekerjaan\tPilih pekerjaan yang tersedia\n", disnaker_str);
            ...
            Dialog_Show(playerid, DISNAKER_MENU, DIALOG_STYLE_TABLIST_HEADERS, ""TTR"Astral City Roleplay "WHITE"- Disnaker Menu", disnaker_str, "Pilih", "Batal");
        }
    }
    return 1;
}
