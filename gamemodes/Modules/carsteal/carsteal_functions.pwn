...
CMD:csinfo(playerid, params[])
{
    ...
    if(IsDurringCarSteal(playerid))
    {
        ...
        if(GetPVarInt(playerid, "CsInfo") == 1) // Sultan
        {
            ...
            ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""TTR"Astral City Roleplay "WHITE"- Cari Kendaraan", frmxt, "Tutup", "");
        }
...
CMD:carsteal(playerid, params[])
{
    ...
    if(IsPlayerInDynamicArea(playerid, Steal_Stuffs[carstealStartArea]))
    {
        if(!PlayerHasItem(playerid, "Linggis"))
        {
            ...
            ShowPlayerDialog(playerid, DIALOG_CARSTEAL_SHOP, DIALOG_STYLE_TABLIST_HEADERS, ""TTR"Astral City Roleplay "WHITE"- Steal Shop",
            "Nama Item\tHarga\
            \nLinggis\t$1000\
...
hook OnPlayerStateChange(playerid, newstate, oldstate)
{
    if(newstate == PLAYER_STATE_DRIVER && oldstate == PLAYER_STATE_ONFOOT)
    {
        ...
        if(vehicleid == CarStealVehicle[playerid])
        {
            ...
            TextDrawSetString(RobberyGlobalTD[3], "Astral City Roleplay~n~Car Stealing");
