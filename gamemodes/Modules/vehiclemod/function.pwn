...
Vehicle_ObjectAddObjects(playerid, vehicleid, model, type)
{
    ...
    { 
        if(VehicleObjects[vehicleid][slot][vehObjectExists] == false)
        {
            ...
            Dialog_Show(playerid, DIALOG_MODSHOPMOVE, DIALOG_STYLE_LIST, ""TTR"Astral City Roleplay "WHITE"- Modshop", "Position\nPosition (Manual)", "Select", "Close");
...
Vehicle_SpotLightAdd(playerid, vehicleid, model, type)
{
    ...
    { 
        if(VehicleObjects[vehicleid][slot][vehObjectExists] == false)
        {
            ...
            Dialog_Show(playerid, DIALOG_SPOTLIGHT, DIALOG_STYLE_LIST, ""TTR"Astral City Roleplay "WHITE"- Modshop", "Position\nPosition (Manual)\nLight Color", "Select", "Close");
    ...
    if(dialogid == DIALOG_MODSHOP)
    {
        if(response)
        {
            switch(listitem)
            {
                ...
                case 5:
                {
                    Dialog_Show(playerid, MODSHOP_CUSTOM, DIALOG_STYLE_INPUT, ""TTR"Astral City Roleplay "WHITE"- Modshop",
