...
new PlayerText:TwitterList[MAX_PLAYERS][3][4];
...
stock CreateTwitterList(playerid) {
    ...
    for(new idx = 0; idx < 3; idx++) {
        new Float:y = (idx * 43.0);

        TwitterList[playerid][idx][0] = CreatePlayerTextDraw(playerid, 477.000, 180.000 + y, "LD_BUM:blkdot");
        PlayerTextDrawTextSize(playerid, TwitterList[playerid][idx][0], 109.000, 40.000);
        PlayerTextDrawAlignment(playerid, TwitterList[playerid][idx][0], 1);
        PlayerTextDrawColor(playerid, TwitterList[playerid][idx][0], 406118911);
        ...
        TwitterList[playerid][idx][1] = CreatePlayerTextDraw(playerid, 481.000, 182.000  + y, "Rayy_Bonelo");
        PlayerTextDrawLetterSize(playerid, TwitterList[playerid][idx][1], 0.109, 0.800);
        PlayerTextDrawAlignment(playerid, TwitterList[playerid][idx][1], 1);
        PlayerTextDrawColor(playerid, TwitterList[playerid][idx][1], -1);
        ...
        TwitterList[playerid][idx][2] = CreatePlayerTextDraw(playerid, 481.000, 190.000 + y, "anjaya ilove you wkkwkwkw tolol bange tjembut wkwkkw taik");
        PlayerTextDrawLetterSize(playerid, TwitterList[playerid][idx][2], 0.109, 0.800);
        PlayerTextDrawTextSize(playerid, TwitterList[playerid][idx][2], 574.000, 0.000);
        PlayerTextDrawAlignment(playerid, TwitterList[playerid][idx][2], 1);
        PlayerTextDrawColor(playerid, TwitterList[playerid][idx][2], -1);
        ...
        TwitterList[playerid][idx][3] = CreatePlayerTextDraw(playerid, 583.000, 209.000 + y, "6s Age");
        PlayerTextDrawLetterSize(playerid, TwitterList[playerid][idx][3], 0.109, 0.800);
        PlayerTextDrawAlignment(playerid, TwitterList[playerid][idx][3], 3);
        PlayerTextDrawColor(playerid, TwitterList[playerid][idx][3], -1);
...
hook ClickDynPlayerTextdraw(playerid, PlayerText:textid) {
    if(textid == TextDraw_Phone[playerid][34])
    {
    ...
    if(textid == TextDraw_PhoneTwitter[playerid][38]) {
        Dialog_Show(playerid, TwitterPost, DIALOG_STYLE_INPUT, ""TTR"Astral City Roleplay{FFFFFF} - X", ""WHITE"Ketik sesuatu untuk tweet anda:", "Post", "Close");
    }
    if(textid == TextDraw_PhoneTwitter[playerid][39]) 
    {
    ...
    if(textid == TextDraw_PhoneTwitter[playerid][40]) 
    {
        pageTwitter[playerid]++;
...
Dialog:TwitterPost(playerid, response, listitem,inputtext[]) {
    if(response) {
        ...
        if(isnull(inputtext))
            return Dialog_Show(playerid, TwitterPost, DIALOG_STYLE_INPUT, ""TTR"Astral City Roleplay{FFFFFF} - Twitter", ""WHITE"Ketik sesuatu untuk tweet anda:", "Post", "Close");
...
task DelayTwitter[1000]()
{
    for(new i; i < MAX_PLAYERS; i++)
    {
        if(AccountData[i][pDelayTwitter] > 0)
        {
            AccountData[i][pDelayTwitter]--;
        }
    }
    return 1;
}