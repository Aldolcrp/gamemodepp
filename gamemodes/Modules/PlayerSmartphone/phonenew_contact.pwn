...
new PlayerText:Contact_Head[MAX_PLAYERS][7],
    PlayerText:Contact_Select[MAX_PLAYERS][7],
    PlayerText:Contact_Body[MAX_PLAYERS][7],
    PlayerText:Contact_Info[MAX_PLAYERS][7],
    PlayerText:Contact_Line[MAX_PLAYERS][7];
...
CreateContactList(playerid) 
{
    for(new idx = 0; idx < 7; idx++) {
        new Float: y = (idx * 21.0);
        
        Contact_Select[playerid][idx] = CreatePlayerTextDraw(playerid, 475.000, 185.000 + y, "LD_BUM:blkdot");
        PlayerTextDrawTextSize(playerid, Contact_Select[playerid][idx], 113.000, 19.000);
        PlayerTextDrawAlignment(playerid, Contact_Select[playerid][idx], 1);
        PlayerTextDrawColor(playerid, Contact_Select[playerid][idx], 2005441023);
        ...
        Contact_Body[playerid][idx] = CreatePlayerTextDraw(playerid, 475.000, 194.000 + y, "LD_BEAT:chit");
        PlayerTextDrawTextSize(playerid, Contact_Body[playerid][idx], 15.000, 8.000);
        PlayerTextDrawAlignment(playerid, Contact_Body[playerid][idx], 1);
        PlayerTextDrawColor(playerid, Contact_Body[playerid][idx], 793726975);
        ...
        Contact_Head[playerid][idx] = CreatePlayerTextDraw(playerid, 477.000, 188.000 + y, "LD_BEAT:chit");
        PlayerTextDrawTextSize(playerid, Contact_Head[playerid][idx], 10.000, 8.000);
        PlayerTextDrawAlignment(playerid, Contact_Head[playerid][idx], 1);
        PlayerTextDrawColor(playerid, Contact_Head[playerid][idx], 793726975);
        ...
        Contact_Line[playerid][idx] = CreatePlayerTextDraw(playerid, 491.000, 194.000 + y, "LD_BUM:blkdot");
        PlayerTextDrawTextSize(playerid, Contact_Line[playerid][idx], 96.000, 1.000);
        PlayerTextDrawAlignment(playerid, Contact_Line[playerid][idx], 1);
        PlayerTextDrawColor(playerid, Contact_Line[playerid][idx], 793726975);
        ...
        Contact_Info[playerid][idx] = CreatePlayerTextDraw(playerid, 492.000, 185.000 + y, "Rayy Bonelo~n~12345678910");
        PlayerTextDrawLetterSize(playerid, Contact_Info[playerid][idx], 0.150, 0.999);
        PlayerTextDrawAlignment(playerid, Contact_Info[playerid][idx], 1);
        PlayerTextDrawColor(playerid, Contact_Info[playerid][idx], 793726975);
...
stock PhoneContactSync(playerid)
{
    ...
    new Contact_Names[7][32],
        Contact_Numbers[7][64],
        Contact_Status[7],
        Contact_Exists[7],
        Contact_Id[7];
    ...
    for (new i = index; i < index + 7 && ii < sizeof(Contact_Exists); i++) {
        if (i < MAX_CONTACTS) {
            if (ContactData[playerid][i][contactExists]) {
                Contact_Exists[ii] = 1;
...
stock ShowContactMenu(playerid) 
{
    ...
    Dialog_Show(playerid, ContactMenu, DIALOG_STYLE_LIST, ""TTR"Astral City Roleplay{FFFFFF} - Contact Menu", string, "Select", "Close");
    return 1;
}
...
Dialog:ContactMenu(playerid, response, listitem, inputtext[]) {
    if(response) {
        ...
        if(listitem == 2) 
        {
            Dialog_Show(playerid, EditContact, DIALOG_STYLE_LIST, ""TTR"Astral City Roleplay{FFFFFF} - Edit Contact", "Edit Name\nEdit Number", "Select", "Close");
        }
        if(listitem == 3) 
        {
...
Dialog:ContactAdd(playerid, response, listitem, inputtext[]) {
    if (response) {
        if (!IsNumeric(inputtext)) {
            return Dialog_Show(playerid, ContactAdd, DIALOG_STYLE_INPUT, ""TTR"Astral City Roleplay{FFFFFF} - Contact Add", "Masukkan nomor kontak yang valid:", "Next", "Close");
        }
		if(IsContactHas(playerid, inputtext)) {
            return Dialog_Show(playerid, ContactAdd, DIALOG_STYLE_INPUT, ""TTR"Astral City Roleplay{FFFFFF} - Contact Add", "Nomor sudah dimiliki!\nMasukkan nama kontak:", "Submit", "Close");
		}

        SetPVarString(playerid, "NumberC", inputtext);

        Dialog_Show(playerid, ContactAddName, DIALOG_STYLE_INPUT, ""TTR"Astral City Roleplay{FFFFFF} - Contact Add", "Masukkan nama kontak:", "Submit", "Close");
    }
    return 1;
}
...
Dialog:EditContact(playerid, response, listitem, inputtext[]) {
    if(response) {
        if(listitem == 0) {
            Dialog_Show(playerid, EditContactName, DIALOG_STYLE_INPUT, ""TTR"Astral City Roleplay{FFFFFF} - Edit Contact Name", "Silahkan masukkan nama kontak yang baru:", "Edit", "Close");
        }
        if(listitem == 1) {
            Dialog_Show(playerid, EditContactNumber, DIALOG_STYLE_INPUT, ""TTR"Astral City Roleplay{FFFFFF} - Edit Contact Number", "Silahkan masukkan nomor kontak yang baru:", "Edit", "Close");
        }
    }
    return 1;
}
...
Dialog:EditContactName(playerid, response, listitem, inputtext[]) {
    if(response) {
        ...
        if(isnull(inputtext))
            return Dialog_Show(playerid, EditContactName, DIALOG_STYLE_INPUT, ""TTR"Astral City Roleplay{FFFFFF} - Edit Contact Name", "Silahkan masukkan nama kontak yang baru:", "Edit", "Close");
...
Dialog:EditContactNumber(playerid, response, listitem, inputtext[]) {
    if (response) {
        ...
        if (isnull(inputtext) || strlen(inputtext) == 0)
            return Dialog_Show(playerid, EditContactNumber, DIALOG_STYLE_INPUT, ""TTR"Astral City Roleplay{FFFFFF} - Edit Contact Number", "Silahkan masukkan nomor kontak yang baru:", "Edit", "Close");

        if (!IsNumeric(inputtext))
            return Dialog_Show(playerid, EditContactNumber, DIALOG_STYLE_INPUT, ""TTR"Astral City Roleplay{FFFFFF} - Edit Contact Number", "Nomor yang dimasukkan tidak valid. Silahkan masukkan nomor kontak yang baru:", "Edit", "Close");

        if (IsContactHas(playerid, inputtext))
            return Dialog_Show(playerid, EditContactNumber, DIALOG_STYLE_INPUT, ""TTR"Astral City Roleplay{FFFFFF} - Edit Contact Number", "ERROR: Nomor ini sudah ada pada kontakmu!\nSilahkan masukkan nomor kontak yang baru:", "Edit", "Close");
...
{
	if(textid == TextDraw_PhoneCall[playerid][38]) // RED BUTTON 
    {
	...
	if(textid == TextDraw_PhoneCall[playerid][39]) // GREEN BUTTON
    {
        ...
        foreach(new i : Player) if (AccountData[i][phoneCallingWithPlayerID] == playerid)
        {
            ...
            PlayerTextDrawSetString(playerid, TextDraw_PhoneCall[playerid][36], IsNumberKnow(playerid, AccountData[callingWithPlayerID][pPhone]));
            ...
            PlayerTextDrawSetString(playerid, TextDraw_PhoneCall[playerid][37], "In Call...");
            PlayerTextDrawSetString(playerid, TextDraw_PhoneCall[callingWithPlayerID][37], "In Call...");
            ...
            PlayerTextDrawHide(i, TextDraw_PhoneCall[i][38]);

            PlayerTextDrawHide(i, TextDraw_PhoneCall[i][39]);
            PlayerTextDrawShow(i, TextDraw_PhoneCall[i][40]);	

            break;
        }
	}
	if(textid == TextDraw_PhoneCall[playerid][40]) // RED BUTTON WHEN IN CALL
    {
        CutCallingLine(playerid); 
	}

    if(textid == TextDraw_PhoneCallNumber[playerid][34]) {
    ...
    if(textid == TextDraw_Phone[playerid][41]) 
    {
    ...
    if(textid == TextDraw_Contact[playerid][22]) 
    {
        HideContactList(playerid);
        PhoneContacts(playerid, false);
        Phone(playerid, true);
    }
    if(textid == TextDraw_Contact[playerid][41]) 
    {
        Dialog_Show(playerid, ContactAdd, DIALOG_STYLE_INPUT, ""TTR"Astral City Roleplay{FFFFFF} - Contact Add", "Masukkan nomor kontak:", "Next", "Close");
    }
    if(textid == TextDraw_Contact[playerid][45]) 
    {
    ...
    if(textid == TextDraw_Contact[playerid][46]) 
    {
        contactPage[playerid]++;
    ...
    if(GetPVarInt(playerid, "OpenContact")) {
        for(new i = 0; i < 7; i++) if(textid == Contact_Select[playerid][i]) 
        {
...
function OnIncomingCall(playerid, fromnumber[])
{
    ...
    PlayerTextDrawSetString(playerid, TextDraw_PhoneCall[playerid][36], IsNumberKnow(playerid, fromnumber));
    PlayerTextDrawSetString(playerid, TextDraw_PhoneCall[playerid][37], "Panggilan masuk...");
	...
	PlayerTextDrawShow(playerid, TextDraw_PhoneCall[playerid][38]);
	PlayerTextDrawShow(playerid, TextDraw_PhoneCall[playerid][39]);
	PlayerTextDrawHide(playerid, TextDraw_PhoneCall[playerid][40]);	
    return 1;
}
...
function OnOutcomingCall(playerid, targetnumber[])
{
    ...
    PlayerTextDrawSetString(playerid, TextDraw_PhoneCall[playerid][36], IsNumberKnow(playerid, targetnumber));
    PlayerTextDrawSetString(playerid, TextDraw_PhoneCall[playerid][37], "_");
    ...
    if(IsPlayerConnected(targetnumberownerid) && targetnumberownerid != INVALID_PLAYER_ID)
    {
        ...
        {
            PlayerTextDrawSetString(playerid, TextDraw_PhoneCall[playerid][37], "Panggilan sedang sibuk...");
        ...
        else
        {
            ...
            PlayerTextDrawSetString(playerid, TextDraw_PhoneCall[playerid][37], "Berdering...");
        }
    }
